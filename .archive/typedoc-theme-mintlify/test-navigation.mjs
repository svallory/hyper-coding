...
function validateStructure(result, groupings, sidebarIcons) {
  ...
  if (groupings.includes('kind') && groupings.length === 1) {
    ...
    const hasKindIcons = result.some(item =>
      typeof item === 'object' && item.icon &&
      ['cube', 'plug', 'bolt'].includes(item.icon)
    );
    if (!hasKindIcons) {
  ...
  if (groupings.includes('folder') && groupings.includes('file') && groupings.length === 2) {
    if (groupings[0] === 'folder') {
      ...
      const hasFileSubGroup = result.some(item => 
        typeof item === 'object' && item.group && 
        ['Actions', 'Recipe Engine'].includes(item.group) &&
        item.pages && item.pages.some(page => 
          typeof page === 'object' && page.group &&
          ['action-tool', 'recipe-engine', 'step-executor', 'types', 'utils'].includes(page.group)
        )
      );
      ...
      const hasFolderSubGroup = result.some(item => 
        typeof item === 'object' && item.group && 
        ['action-tool', 'recipe-engine', 'step-executor', 'types', 'utils'].includes(item.group) &&
        item.pages && item.pages.some(page => 
          typeof page === 'object' && page.group &&
