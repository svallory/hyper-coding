...
export function transformToMintlify(page: MarkdownPageEvent<any>): string {
...
function generateFrontmatter(page: MarkdownPageEvent<any>): string {
	...
	if (model.comment?.summary) {
		description = model.comment.summary
			.map((part: any) => part.text || part.toString())
			.join(" ")
			.replace(/\n/g, " ")
			.trim()
			.slice(0, 160);
...
function transformComponents(content: string, page: MarkdownPageEvent<any>): string {
...
function groupCodeExamples(content: string): string {
	...
	return content.replace(
		...
		(match, block1, block2) => {
			...
			if (
				(block1.includes("typescript") || block1.includes("javascript")) &&
