...
// Note: Using post-generation file scanning approach instead of project reflection predictions
...
export function load(app: Application) {
  ...
  // Note: No longer capturing project reference since we scan actual output files instead

  // Transform markdown content to Mintlify-compatible MDX after it's rendered
  const markdownApp = app as unknown as MarkdownApplication;
  ...
  markdownApp.renderer.on(MarkdownRendererEvent.END, (event) => {
    ...
    const sidebarIcons = app.options.getValue('mintlifySidebarIcons') as Array<"folder" | "file" | "kind"> | "all" | "none";
    const includeFileExtensions = app.options.getValue('mintlifyIncludeFileExtensions') as boolean;
    ...
    if (docsJsonPath) {
      try {
        ...
        // Use post-generation approach to scan actual files and eliminate broken links
        updater.updateNavigationPostGeneration();
        
        // Note: Old reflection-based method removed to eliminate broken link predictions
      } catch (error) {
