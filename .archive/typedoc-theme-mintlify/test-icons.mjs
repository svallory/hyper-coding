...
function validateIcons(result, sidebarIcons, groupings) {
	...
	function analyzeNode(node, expectedLevel) {
		...
		if (node.icon) {
			...
			if (!shouldHaveIcon) {
				issues.push(
					`Unexpected icon '${node.icon}' on ${expectedLevel} level group '${node.group}'`,
				);
			} else {
				// Count icon types
...
for (const iconConfig of ["all", "none", ["kind"], ["folder"]]) {
	...
	updaterSingle.buildHierarchicalStructure =
		updaterSingle.constructor.prototype.buildHierarchicalStructure;

	try {
		const result = updaterSingle.buildHierarchicalStructure.call(
			updaterSingle,
