...
describe(`typedoc-plugin-markdown (Integration /Comments)`, () => {
	...
	it(`should handle single example tags`, () => {
		expectFileToEqual("comments", ["members"], ["/functions/singleExampleTag.md"]);
	});

	it(`should handle multiple example tags`, () => {
		expectFileToEqual("comments", ["members"], ["/functions/multipleExampleTags.md"]);
	});
	...
	it(`should handle const function with returns`, () => {
		expectFileToEqual("comments", ["members"], ["/functions/constFunctionWithReturns.md"]);
	});
