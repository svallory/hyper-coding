...
describe(`typedoc-vitepress-theme`, () => {
	it(`should output docs with vitepress theme`, async () => {
		const contents = fs.readFileSync(path.join(__dirname, "../out/default/index.md")).toString();
		assertToMatchSnapshot("index-page", contents);
	});

	it(`should output members that require anchor slugification`, async () => {
		const contents = fs
			.readFileSync(path.join(__dirname, "../out/default/module-1/interfaces/InterfaceA.md"))
			.toString();
	...
	it(`should generate typedoc sidebar with options`, async () => {
		const contents = fs
			.readFileSync(path.join(__dirname, "../out/sidebar-options/typedoc-sidebar.json"))
			.toString();
	...
	it(`should generate typedoc sidebar with duplicate out and docsRoots`, async () => {
		const contents = fs
			.readFileSync(path.join(__dirname, "../out/sidebar-options-2/typedoc-sidebar.json"))
			.toString();
