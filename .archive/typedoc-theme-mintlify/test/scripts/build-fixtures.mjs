...
async function main() {
	const allConfigs = fs.readdirSync(path.join(__dirname, "..", "fixtures", "configs"));
	...
	const tasks = typedocConfigs.map((config) => limit(() => processFixture(config)));
...
export async function processFixture(config) {
	...
	const cmdArgs = ["-options", path.join(__dirname, "..", "fixtures", "configs", config)];
