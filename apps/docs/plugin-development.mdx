...
import { HypergenPlugin, PluginContext, PluginMetadata } from '@hyperdev/hypergen';

export class MyCustomPlugin implements HypergenPlugin {
  // Plugin metadata
...
import { Action, ActionContext, ActionResult } from '@hyperdev/hypergen';
...
import { CLIPlugin, CommandContext, CommandDefinition } from '@hyperdev/hypergen';
...
export class CustomCLIPlugin implements CLIPlugin {
  ...
  private async authMiddleware(context: CommandContext, next: () => Promise<void>) {
    ...
    if (this.requiresAuth(context.command)) {
      ...
      if (!isAuthenticated) {
        console.error('Authentication required. Run `hypergen login` first.');
        process.exit(1);
      }
    }
...
```json
{
  "name": "@yourorg/hypergen-custom-plugin",
  "version": "1.0.0",
  "description": "Custom HyperDev plugin for specialized workflows",
  "keywords": ["hypergen", "plugin", "code-generation"],
...
npm install -g @yourorg/hypergen-custom-plugin

# Install plugin from local directory
hypergen plugin install ./my-plugin

# List installed plugins
hypergen plugin list

# Configure plugin
hypergen plugin configure @yourorg/hypergen-custom-plugin
```

```javascript
// hypergen.config.js - Plugin configuration
export default {
  plugins: [
    // Simple plugin registration
    '@yourorg/hypergen-custom-plugin',
