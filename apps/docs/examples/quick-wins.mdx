...
export const <%= componentName %> = () => {
  return (
    <div className={`<%= componentName.toLowerCase() %> ${className || ''}`} {...props}>
      {children || 'Hello from <%= componentName %>!'}Hello from <%= componentName %>!
    </div>
  );
};
...
**Storybook Stories** (`templates/Component.stories.tsx.ejs.t`)
...
router.post(
  '/', 
  authenticate,
  create<%= h.inflection.classify(resourceName) %>Validation,
...
router.put(
  '/:id', 
  authenticate,
  update<%= h.inflection.classify(resourceName) %>Validation,
...
---
{
  ...
  "exclude": [
    ...
    "build",
    "coverage"
    // Note: coverage line is conditional in the template
  ]
}
```

**Build Configuration** (`templates/tsconfig.build.json.ejs.t`)
```json
---
...
**Test Configuration** (`templates/tsconfig.test.json.ejs.t`)
```json
---
...
---
{
  ...
  "scripts": {
    ...
    "dev": "next dev<% } else if (projectType === 'api') { %>nodemon src/index.<%= useTypeScript ? 'ts' : 'js' %>ts-node src/index.ts",
    "start": "next start<% } else if (useTypeScript && projectType !== 'library') { %>node dist/index.jsnode src/index.js",
    "test": "jest",
...
**Test Setup** (`templates/setup.ts.ejs.t`)
```typescript
---
to: tests/setup.ts
---
