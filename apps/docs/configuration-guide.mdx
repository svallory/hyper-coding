...
The primary configuration file for HyperDev projects is `hypergen.config.js`:
...
export default {
  ...
  // Extensions
  plugins: [
    '@hypergen/plugin-typescript',
    '@hypergen/plugin-react',
...
<Accordion title="Configuration File Search Order">
  ...
  2. **Environment variable**: `HYPERGEN_CONFIG=path/to/config.js`
  ...
  5. **Home directory**: `~/.hypergenrc` or `~/.config/hypergen/config.js`
  6. **System-wide**: `/etc/hypergen/config.js` (Linux/macOS)
</Accordion>
...
hypergen:
  ...
  sharedTemplates: '.hypergen/shared'
...
```bash
# Template discovery
export HYPERGEN_TEMPLATES="./my-templates,./shared-templates"
export HYPERGEN_DISCOVERY_SOURCES="local,npm"

# Engine settings
export HYPERGEN_ENGINE_TYPE="handlebars"
export HYPERGEN_CONFLICT_STRATEGY="merge"

# Performance
export HYPERGEN_CACHE_ENABLED="true"
export HYPERGEN_CACHE_TTL="7200000"

# Security
export HYPERGEN_ALLOW_REMOTE="false"
export HYPERGEN_TRUSTED_SOURCES="npm:@myorg/*,github:myorg/*"

# Development
export NODE_ENV="development"
export HYPERGEN_DEBUG="true"
export HYPERGEN_VERBOSE="true"
```
...
```javascript config/hypergen.prod.js
...
export default {
  ...
  discovery: {
    directories: ['~/templates', '~/.hypergen/templates']
  },
...
```json ~/.config/hypergen/config.json
{
  "defaults": {
    "author": "John Doe",
    "license": "MIT"
...
export default {
  plugins: [
    ...
    // Plugin with options
    {
      plugin: '@hypergen/plugin-database',
      options: {
        provider: 'postgresql',
        migrations: true
      }
...
```javascript
import { validateConfig } from 'hypergen'
...
{
  "scripts": {
    "config:validate": "hypergen validate-config",
    "config:init": "hypergen init-config --interactive"
  }
}
```
...
hypergen init --interactive

# Upgrade existing configuration
hypergen config upgrade
...
<Tabs>
  <Tab title="Frontend Project">
    ```bash
    hypergen init --template=react-app
    hypergen init --template=vue-app
    hypergen init --template=angular-app
    ```
  </Tab>
  <Tab title="Backend Project">
    ```bash
    hypergen init --template=node-api
    hypergen init --template=fastify-api
    hypergen init --template=express-app
    ```
  </Tab>
  <Tab title="Full-Stack">
    ```bash
    hypergen init --template=next-app
    hypergen init --template=nuxt-app
    hypergen init --template=t3-stack
    ```
  </Tab>
</Tabs>
...
hypergen migrate --from=1.0 --to=2.0

# Show migration preview
hypergen migrate --dry-run

# Backup before migration
hypergen migrate --backup
```
...
<AccordionGroup>
  <Accordion title="Configuration Not Found">
    ...
    **Solutions**:
    - Check file name spelling: `hypergen.config.js` (not `hypergen.config.ts`)
  ...
  <Accordion title="Invalid Configuration Format">
    ...
    **Solutions**:
    - Validate JSON syntax with `hypergen config validate`
    - Check JavaScript syntax with `node -c hypergen.config.js`
  ...
  <Accordion title="Environment Override Not Working">
    ...
    **Solutions**:
    - Check variable naming: `HYPERGEN_CACHE_ENABLED` not `CACHE_ENABLED`
    - Verify environment is correctly detected: `NODE_ENV=production`
    - Use `hypergen config show` to see resolved configuration
...
DEBUG=hypergen:config hypergen generate

# Show resolved configuration
hypergen config show --verbose

# Validate configuration
hypergen config validate --strict
```
