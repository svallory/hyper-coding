...
- The question text (with template variables already resolved)
...
The tag takes an `output` parameter that defines the expected answer shape. The body is the question text — it supports full Jig syntax, so you can include dynamic context.
...
@answer
  <div className="grid grid-cols-3">
    <div className="col-span-2">
      @each(field in answer.main)
      <FormField name="{{ field }}" />
      @end
    </div>
    <aside>
      @each(field in answer.sidebar)
      <FormField name="{{ field }}" />
      @end
    </aside>
  </div>
...
```jig
---
to: "app/admin/{{ pluralize :: downcase :: model.name }}/[id]/page.tsx"
---
import type { {{ model.name }} } from "@/types";
@each(rel in model.relations)
import type { {{ rel.model }} } from "@/types";
@end

{{-- --- AI Decision: Field grouping --- --}}
@let(editableFields = model.fields | where: "readonly", false)
@let(fieldNames = editableFields | map: "name")

@ai(output: { kind: "group", from: fieldNames, groups: ["primary", "secondary", "advanced"] })
  This is an admin edit page for {{ model.name }}.
  Fields: {{ join: ", " :: map: "name" :: editableFields }}.
  Types: {{ join: ", " :: map: "type" :: editableFields }}.
...
export default async function {{ model.name }}EditPage({
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <section>
        <div className="grid grid-cols-2 gap-4">
          @each(field in answer.secondary)
          <FormField name="{{ field }}" />
          @end
        </div>
      </section>
      ...
      <Tabs defaultValue="{{ map: 'name' :: first :: model.relations }}">
        <TabsList>
          @each(rel in model.relations)
...
@if(model.relations.size > 0)
@let(relationNames = model.relations | map: "name")

@ai(output: { kind: "map", from: relationNames, to: listComponents })
  The {{ model.name }} model has these relations:
  @each(rel in model.relations)
  - {{ rel.name }}: {{ rel.type }} {{ rel.model }} (fields: {{ join: ", " :: rel.fields }})
  @end

  Available list/table components: {{ join: ", " :: listComponents }}.
  Which component should render each relation?
@end
@end
```
