...
- **`@ai` template tag** — Asks the AI agent to make **decisions** (pick components, order fields, group items). The template renders the output deterministically using the answers. See [AI Decisions in Templates](/docs/kits/ai-decisions-tag).
...
**Why?** An AI agent calling `hyper nextjs page add` already knows what it wants. It will edit the generated file directly. Having Hypergen's AI step rewrite the file first is redundant — the agent would rewrite it again anyway.

Templates earn their keep here because they handle **conventions**: correct file placement, proper imports, framework-specific patterns (like Next.js `params` being a `Promise`), and TypeScript types. That's deterministic work that doesn't need an LLM.
...
steps:
  - name: Generate CRUD scaffold
    tool: template
    template: templates/crud-scaffold.jig
...
When a company has specific patterns every file must follow — auth wrappers, logging setup, analytics hooks, error boundaries — the template bakes those in. The AI only fills the business logic inside the convention scaffold.

This works especially well when the conventions are complex enough that describing them in every prompt would be error-prone:
...
A "feature" recipe that generates 8+ files with correct cross-references — barrel exports, route registrations, navigation entries, test files importing the right modules. Templates handle the wiring; AI fills each file's content.
...
When you do use `tool: ai`, follow these guidelines:
  ...
  <details>
<summary>Use inject, not overwrite</summary>
  ...
  <details>
<summary>Always set guardrails</summary>
  ...
  <details>
<summary>Keep temperature low</summary>
  ...
  <details>
<summary>The scaffold is your fallback</summary>
