...
<Aside type="note">The `name` field must match the recipe's directory name in the cookbook structure.</Aside>
...
<Aside type="tip">Use `multiple: true` for enum variables when users should select multiple options. This is perfect for feature flags or selecting which components to generate.</Aside>
...
<Aside type="note">Template paths are relative to the recipe directory. Templates use the `.jig` extension.</Aside>
...
<Aside type="caution">Shell commands run in the project root. Use caution with destructive operations.</Aside>
...
<Aside type="tip">Use `parallel` for independent steps like generating multiple files. Use `sequence` when steps depend on each other.</Aside>
...
<Aside type="note">AI steps are separate from the `@ai` tags inside templates. Template `@ai` tags handle code-level generation, while AI steps generate entire content blocks.</Aside>
...
<Aside type="note">`args:` supports Jig template syntax for transforming variables. You can rename, combine, or compute new values.</Aside>
...
<Aside type="caution">`when:` conditions use plain JavaScript expressions — do NOT wrap them in `{{ }}`. The expression has access to all variables in the current context.</Aside>

### Conditional Examples

```yaml title="Boolean Check"
...
### Benefits of Meta-Recipes
  ...
  <details>
<summary>Granular Control</summary>

    Each sub-recipe is independently callable:

    ```bash
    # Just the list page
    hyper nextjs crud list-page Organization

    # Just create and edit
    hyper nextjs crud resource Product --pages=create,edit
    ```

  </details>

  <details>
<summary>Efficient Batch Generation</summary>

    The meta-recipe generates everything at once:

    ```bash
    # All pages with AI assistance
    hyper nextjs crud resource Organization --ask=ai
    ```
...
<Aside type="tip">Start with granular recipes for individual features, then create meta-recipes to combine them. This gives users flexibility while saving time on common workflows.</Aside>
...
<Steps>
1. **Variables with defaults are used as-is**
    ```yaml
    variables:
      includeAuth:
        type: boolean
...
2. **Variables with suggestions get AI-informed values**
    ```yaml
    variables:
      port:
        type: number
...
3. **Variables without values get AI-generated values**
    ```yaml
    variables:
      name:
        type: string
...
```yaml title="Good: Clear Description"
variables:
  name:
    type: string
    required: true
...
```yaml title="Good: Helpful Suggestion"
variables:
  tableColumns:
    type: array
...
```yaml title="Bad: Vague Description"
variables:
  name:
    type: string
...
```yaml title="Bad: Suggestion as Default"
variables:
  port:
    type: number
...
```bash
# Instead of: hyper nextjs crud resource --name=Organization
hyper nextjs crud resource Organization

# With multiple positional args:
hyper nextjs crud resource Organization update
```

<Aside type="tip">Use `position: 0` for the most important required variable (usually a name). Reserve higher positions for optional or action-type variables.</Aside>
...
<Aside type="caution">Positional arguments are zero-based. `position: 0` is the first argument, `position: 1` is the second, etc.</Aside>
...
<Aside type="tip">Use this structure as a starting point for your own meta-recipes, then break down complex steps into their own sub-recipes for better reusability.</Aside>
