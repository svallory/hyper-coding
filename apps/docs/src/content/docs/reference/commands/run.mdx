...
<Aside type="note">
`hyper run` is an alias for [`hyper gen`](/reference/commands/gen). Both accept the same arguments and flags. This page focuses on **variable resolution** — how recipe variables get their values.
</Aside>
...
```bash
hyper run <recipe> [options] [-- key=value ...]
```

Or using kit shorthand:

```bash
hyper <kit> <cookbook> <recipe> [positional-args] [--key=value ...]
```
...
When a recipe declares variables, hyper resolves their values through a pipeline. Two flags control how this works: **`--ask`** and **`--no-defaults`**.
...
<Tabs>
  <TabItem label="me (default in TTY)">
    ...
    ```bash
    hyper nextjs crud resource --name=organization
  ...
  <TabItem label="ai">
    ...
    ```bash
    hyper nextjs crud resource --name=organization --ask=ai
    # AI infers field types, relationships, etc.
    ```
  </TabItem>
  <TabItem label="nobody">
    No resolution at all. Errors if any required variable is missing. Ideal for CI/CD pipelines and scripting.

    ```bash
    hyper nextjs crud resource --name=organization --fields="name:string,slug:string" --ask=nobody
...
<Aside type="caution">
  `default` is mutually exclusive with both `required` and `suggestion`. A variable can't have a default and also be required (the default would always satisfy it), and can't have both a default and a suggestion --- pick one: auto-apply or hint.
</Aside>
...
Provide just the essentials and let AI figure out the rest from context:

```bash
hyper nextjs crud resource \
  --name=organization \
  --ask=ai
```
...
Even variables with defaults go to AI. The AI sees the original defaults as hints but can choose different values.

### Kit shorthand with positional args

```bash
# These are equivalent:
hyper run nextjs/crud/resource --name=organization
hyper nextjs crud resource --name=organization
hyper nextjs crud resource Organization   # positional arg mapped to 'name'
```
...
If neither is available, hyper warns and falls back to interactive prompts (`--ask=me` behavior).

## Related

<CardGrid>
  <Card title="Templates: @ai tag" href="/docs/kits/ai-decisions-tag">
