...
This walkthrough traces every HyperDev feature and hook activation for a single, concrete request: **"Add a user analytics dashboard to track profile engagement metrics."**

It's a realistic scenario --- the kind of task that would normally produce duplicated code and naming inconsistencies. Watch how intrusive thoughts change that.
...
---
flowchart TD
    UserRequest@{ label: "👤 User Request:<br>'Add user analytics dashboard<br>for profile engagement metrics'" } --> SessionStart["🚀 1. SessionStart Hook"]
    SessionStart --> InitDaemon["Initialize intrusive<br>thoughts daemon"] & SetupWatchers["Setup real-time<br>linting watchers"]
    InitDaemon --> IndexExisting["📂 Index existing codebase:<br>• AnalyticsService.ts<br>• UserProfile.tsx<br>• analytics.utils.ts<br>• AdminDashboard.tsx"]
    SetupWatchers --> UserPromptHook["📝 2. UserPromptSubmit Hook"]
    IndexExisting --> UserPromptHook
    UserPromptHook --> QueryDaemon["🔍 Query daemon for<br>similar patterns"]
    QueryDaemon --> ThoughtsFound["Intrusive thoughts<br>found?"]
    ThoughtsFound -- YES --> InjectThoughts["💭 Inject peripheral awareness:<br>• Found AnalyticsService.ts<br>• UserProfile.tsx has metrics<br>• formatMetrics() exists<br>• AdminDashboard.tsx pattern"]
    ThoughtsFound -- NO --> OriginalPlan["🤖 Original Agent Plan:<br>Create new analytics<br>system from scratch"]
    InjectThoughts --> RevisedPlan@{ label: "🧠 Agent revises plan:<br>'Extend existing analytics<br>instead of creating new'" }
    OriginalPlan --> PreToolUse1["⚡ 3a. PreToolUse Hook"]
    ...
    ASTAnalysis1 --> WriteTypes["✍️ Write types/analytics.ts<br>extending BaseMetrics"]
    SyntaxCheck1 --> WriteTypes
    SecurityCheck1 --> WriteTypes
    WriteTypes --> PostTool1["⚡ 4a. PostToolUse Hook"]
    PostTool1 --> LintTypes["Lint types/analytics.ts"] & FormatTypes["Format validation"]
    LintTypes --> TypesOK{"Types valid?"}
    FormatTypes --> TypesOK
    TypesOK -- YES --> PreToolUse2["⚡ 3b. PreToolUse Hook"]
    ...
    AboutToCreate --> DaemonWarns@{ label: "💭 Daemon injection:<br>'AnalyticsService.ts already<br>exists - extend instead!'" }
    DaemonWarns --> AgentPivots@{ label: "🧠 Agent pivots:<br>'I'll extend existing<br>AnalyticsService instead!'" }
    AgentPivots --> ExtendService["✍️ Extend AnalyticsService<br>with trackProfileEngagement()"]
    ExtendService --> PostTool2["⚡ 4b. PostToolUse Hook"]
    PostTool2 --> ServiceOK{"Service extension<br>valid?"}
    ServiceOK -- YES --> PreToolUse3["⚡ 3c. PreToolUse Hook"]
    ...
    AboutComponent --> ConventionWarning@{ label: "💭 Daemon injection:<br>'Project uses PascalCase:<br>UserProfile.tsx, AdminDashboard.tsx<br>Components go in components/dashboards/'" }
    ConventionWarning --> AgentAdjusts@{ label: "🧠 Agent adjusts:<br>'Create UserAnalyticsDashboard.tsx<br>in components/dashboards/'" }
    AgentAdjusts --> WriteComponent["✍️ Write UserAnalyticsDashboard<br>reusing MetricsCard, useAnalytics"]
    WriteComponent --> PostTool3["⚡ 4c. PostToolUse Hook"]
    PostTool3 --> ComponentOK{"Component valid?"}
    ComponentOK -- YES --> ReadyCommit["📋 Ready to commit"]
    ComponentOK -- NO --> FixComponent["Fix component issues"]
    FixComponent --> PreToolUse3
    ReadyCommit --> PreCommit["🔒 5. pre-commit Hook"]
    ...
    AllChecks -- YES --> PrepareMsg["📝 6. prepare-commit-msg Hook"]
    FixIssues --> PreCommit
    PrepareMsg --> SuggestFormat["Suggest conventional<br>commit format"]
    SuggestFormat --> CommitMsg["📝 7. commit-msg Hook"]
    CommitMsg --> ValidateMsg{"Commit message<br>valid?"}
    ValidateMsg -- NO --> FixMsg["Fix commit message"]
    ValidateMsg -- YES --> CreateCommit["📦 Create commit"]
    FixMsg --> CommitMsg
    CreateCommit --> PostCommit["📈 8. post-commit Hook"]
    PostCommit --> UpdateMetrics["Update code metrics:<br>+0 duplications<br>+1 pattern reuse<br>+1 feature extension"] & IndexNewPatterns["Index new patterns:<br>• Analytics extend service<br>• Dashboard naming convention<br>• MetricsCard reuse pattern"]
    UpdateMetrics --> ReadyPush{"Ready to push?"}
    IndexNewPatterns --> ReadyPush
    ReadyPush -- NO --> SessionEnd["🏁 11. SessionEnd Hook"]
    ReadyPush -- YES --> PrePush["🚀 9. pre-push Hook"]
    ...
    PushOK -- YES --> PushCode["📤 Push to remote"]
    FixPushIssues --> PrePush
    PushCode --> CI["🔄 10. CI/CD Pipeline"]
    ...
    SecAudit --> CIResult
    DocsGen --> CIResult
    CIResult -- NO --> CIFail["❌ CI Pipeline failed"]
    CIResult -- YES --> CISuccess["✅ CI Pipeline successful"]
    CISuccess --> SessionEnd
    CIFail --> SessionEnd
    SessionEnd --> SavePatterns["💾 Save learned patterns:<br>• Analytics features extend services<br>• Dashboard components use MetricsCard<br>• formatMetrics utility is reusable"] & ArchiveMetrics["📊 Archive session metrics:<br>85% code reuse<br>0 duplications<br>3 intrusive thoughts acted upon"]
    SavePatterns --> Complete["🎉 Feature Complete"]
...
These patterns carry forward into future sessions --- so the AI gets progressively better at matching your project's conventions without being told every time.
