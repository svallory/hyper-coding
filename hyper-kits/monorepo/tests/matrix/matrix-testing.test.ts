...
class MatrixTesting {
	...
	async generateProject(
		...
		try {
			...
			if (result.generationTimeMs > TEST_CONFIG.maxGenerationTimeMs) {
				result.warnings.push(
					`Generation took ${result.generationTimeMs}ms, exceeding ${TEST_CONFIG.maxGenerationTimeMs}ms target`,
				);
			}
		} catch (error) {
	 ...
	 */
	private async addSamplePackage(
...
`;
	...
	private getLintCommand(config: MonorepoConfig): string {
		switch (config.linter) {
			case "eslint":
				return "eslint src --ext .ts";
			case "biome":
				return "biome check src";
			default:
				return "eslint src --ext .ts";
		}
	}
	...
	private getFormatScript(config: MonorepoConfig): string {
		switch (config.formatter) {
			...
			case "dprint":
				return "dprint fmt";
			case "biome-integrated":
				return "biome format src --write";
			default:
				return "prettier --write src";
		}
	}
				...
				JSON.stringify(
					{
						...
						formatter:
							config.formatter === "biome-integrated"
								? {
										enabled: true,
										indentStyle: "space",
										indentSize: 2,
...
`;
	...
	generatePerformanceReport(): string {
		...
		if (successful.length > 0) {
			...
			report.push(
				`- Combinations exceeding target: ${successful.filter((r) => r.generationTimeMs > TEST_CONFIG.maxGenerationTimeMs).length}\n`,
			);
		}
		...
		if (failed.length > 0) {
			...
			failed.forEach((result) => {
				...
				report.push(
					`- ${combo.packageManager}+${combo.linter}+${combo.formatter}+${combo.testFramework}`,
				);
	...
	async runMatrixTests(): Promise<MatrixTestResult[]> {
		...
		for (let i = 0; i < validCombinations.length; i++) {
			...
			console.log(
				`Testing combination ${i + 1}/${validCombinations.length}: ${combination.packageManager}+${combination.linter}+${combination.formatter}+${combination.testFramework}`,
			);
...
describe("Matrix Testing - All Tool Combinations", () => {
	...
	it(
		"should validate all valid tool combinations",
		async () => {
			const validCombinations = getValidToolCombinations();
	...
	it(
		"should run matrix testing for all tool combinations",
		async () => {
