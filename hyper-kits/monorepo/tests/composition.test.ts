...
describe("getValidToolCombinations", () => {
	it("should return only valid tool combinations", async () => {
		...
		const invalidCombo = combinations.find(
			(c) => c.linter === "eslint" && c.formatter === "biome-integrated",
		);
		expect(invalidCombo).toBeUndefined();

		// Should not include bun-test with non-bun package managers
		const invalidBunTest = combinations.find(
			(c) => c.testFramework === "bun-test" && c.packageManager !== "bun",
		);
	...
	it("should include expected valid combinations", async () => {
		...
		const modernBun = combinations.find(
			(c) =>
		...
		const traditional = combinations.find(
			(c) =>
