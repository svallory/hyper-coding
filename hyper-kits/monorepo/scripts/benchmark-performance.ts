...
import { performanceOptimizer, performanceMonitor, PerformanceBenchmark } from "../src/performance";
...
async function runSingleBenchmark(
	...
	console.log(
		`    ⏱️  Duration: ${duration}ms (target: <${testCase.expectedMaxDuration}ms) ${meetsTarget ? "✅" : "❌"}`,
	);
...
async function runConcurrencyBenchmark(
	...
	const promises = selectedCases.map((testCase, index) =>
		benchmark.benchmarkOperation(testCase.context, `concurrent-${index}`, async () => {
	...
	console.log(
		`    ✅ Success Rate: ${successCount}/${concurrentCount} (${((successCount / concurrentCount) * 100).toFixed(1)}%)`,
	);
...
async function runMemoryBenchmark(
	...
	console.log(
		`    📊 Average per Operation: ${Math.round(totalIncrease / memoryReadings.length)}MB`,
	);
...
async function runCacheBenchmark(
	...
	const thirdRun = await benchmark.benchmarkOperation(
		...
		"cache-third-run",
		async () =>
			performanceOptimizer.composeOptimized({
				...testCase.context,
	...
	console.log(
		`    3️⃣  Variant Run: ${thirdRun.benchmark.durationMs}ms (partial cache hit expected)`,
	);

	if (firstRun.result && secondRun.result) {
		const improvement =
			((firstRun.benchmark.durationMs - secondRun.benchmark.durationMs) /
				firstRun.benchmark.durationMs) *
			100;
		console.log(`    📈 Cache Improvement: ${improvement.toFixed(1)}% faster on second run`);

		const avgHitRate =
			(firstRun.result.cacheStats.hitRate +
				secondRun.result.cacheStats.hitRate +
				(thirdRun.result?.cacheStats.hitRate || 0)) /
			3;
...
async function generateBenchmarkReport(
	...
	const analysisReport = {
		...
		hypergenAnalysis: {
			...
			optimizations: {
				cacheEffectiveness:
					report.results.filter((r) => r.success).length > 0 ? "Active" : "Disabled",
				parallelProcessing: report.results.some((r) => r.operationName.includes("concurrent"))
					? "Active"
					: "Not Used",
				memoryOptimization:
					report.summary.memoryPeakUsage < 512 * 1024 * 1024 ? "Effective" : "Needs Attention",
			},
...
function generateOptimizationRecommendations(report: any): string[] {
	...
	if (report.summary.averageDuration > 30000) {
		recommendations.push(
			"Average duration exceeds 30s target - implement additional optimizations",
		);
	}
	...
	if (fastestCombo) {
		recommendations.push(
			`Fastest configuration: ${fastestCombo.combination} (${fastestCombo.duration}ms)`,
		);
	}
