...
const COMPATIBILITY_MATRIX: CompatibilityMatrix = {
	...
	linter: {
		eslint: {
			...
			warnings: {
				dprint:
					"dprint + ESLint combination may have overlapping rules - consider configuration review",
			},
		},
		biome: {
			...
			warnings: {
				prettier:
					"Using Biome linter with Prettier formatter - consider Biome integrated formatter for better performance",
				dprint:
					"Using Biome linter with dprint formatter - consider Biome integrated formatter for unified toolchain",
			},
		},
	},
	testFramework: {
		vitest: {
			incompatibleWith: {},
			performanceNotes: [
				"Vitest works well with all package managers",
				"Consider Vite for build tooling alignment",
			],
		},
		"bun-test": {
			...
			incompatibleWith: {},
			performanceNotes: [
				"Bun Test offers fastest test execution",
				"Requires Bun as package manager",
			],
		},
		jest: {
			incompatibleWith: {},
			performanceNotes: [
				"Jest has extensive ecosystem support",
				"Consider Vitest for better TypeScript integration",
			],
		},
	},
	combinations: [
		...
		{
			condition: (config) =>
				config.packageManager === "yarn" && config.testFramework === "bun-test",
			severity: "error",
			message: "Bun Test framework requires Bun package manager",
			fix: 'Change package manager to "bun" or select "vitest" or "jest" as test framework',
		...
		{
			condition: (config) =>
				config.packageManager === "pnpm" && config.testFramework === "bun-test",
			severity: "error",
			message: "Bun Test framework requires Bun package manager",
			fix: 'Change package manager to "bun" or select "vitest" or "jest" as test framework',
		...
		},
		{
			...
			severity: "info",
			message:
				"ESLint and dprint combination works well but requires careful configuration to avoid conflicts",
		...
		},
		{
			...
			severity: "info",
			message:
				"Using Jest with Bun package manager works but consider Bun Test or Vitest for better integration",
...
function validateMoonConfig(moon: any): Record<string, any> {
	...
	if (moon.projects !== undefined && Array.isArray(moon.projects)) {
		sanitized.projects = moon.projects.filter(
			(project: any) => typeof project === "string" && project.length > 0,
		);
	}
	...
	try {
		...
		if (minVersion && version) {
			...
			if (currentMajor < minMajor) {
				issues.push(
					`${packageManager} version ${version} is below recommended minimum ${minVersion}`,
				);
			}
		}
	} catch (error: any) {
