...
**Destination:** `packages/hyper-gen/tests/`
...
```bash
# Count test files
find packages/hyper-gen/tests -type f \( -name "*.test.ts" -o -name "*.spec.ts" \) | wc -l

# List all test files
find packages/hyper-gen/tests -type f \( -name "*.test.ts" -o -name "*.spec.ts" \) | sort

# Run tests
cd packages/hyper-gen && bun test
```
