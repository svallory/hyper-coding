 ...
 *
 * Uses the real Next.js kit at /work/hyperdev/hyper-kits/nextjs/cookbooks/crud/resource/
 */
...
describe('E2E: Next.js CRUD resource with 2-pass AI generation', () => {
  ...
  describe('Prompt Assembly', () => {
    it('should produce markdown with all @ai keys', async () => {
      ...
      const prompt = assembler.assemble(collector, {
        originalCommand: 'hypergen nextjs crud resource product --fields="title:string,price:number"',
  ...
  describe('Full Round-Trip', () => {
    it('should complete Pass 1 → Assemble → Pass 2 end-to-end', async () => {
      ...
      const assembler = new PromptAssembler()
      const prompt = assembler.assemble(collector, {
        originalCommand: 'hypergen nextjs crud resource product --fields="title:string,price:number,description:text,isPublished:boolean"',
        answersPath: './ai-answers.json',
      })
      ...
      const schema = await fs.readFile(path.join(tempDir, 'lib/schemas/product-schema.ts'), 'utf-8')
