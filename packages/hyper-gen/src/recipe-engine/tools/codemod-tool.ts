      ...
      case 'namespace':
        importDeclaration = factory.createImportDeclaration(
          ...
          factory.createImportClause(
            false,
            undefined,
            factory.createNamespaceImport(factory.createIdentifier(parameters.alias || parameters.import))
          ),
          factory.createStringLiteral(parameters.from)
        )
        break
    ...
    const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
      return (rootNode) => {
        function visit(node: ts.Node): ts.Node {
          // Add to class
          if (parameters.className && ts.isClassDeclaration(node) && 
            ...
            const newProperty = factory.createPropertyDeclaration(
              ...
              parameters.propertyType ? 
                factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword) : undefined, // Simplified
          ...
          if (parameters.objectName && ts.isVariableDeclaration(node) &&
              ts.isIdentifier(node.name) && node.name.text === parameters.objectName &&
...
export class CodeModTool extends Tool<CodeModStep> {
   ...
   */
  protected async onValidate(step: CodeModStep, context: StepContext): Promise<ToolValidationResult> {
    const errors: string[] = []
    ...
    const validCodeModTypes: CodeModType[] = ['add-import', 'add-export', 'modify-function', 'add-property', 'replace-text', 'custom']
    ...
    if (step.parameters) {
      ...
      switch (step.codemod) {
        case 'add-import':
          ...
          if (params.importType && !['default', 'named', 'namespace', 'side-effect'].includes(params.importType)) {
  ...
  protected async onExecute(
    ...
    try {
      ...
      const stepResult: StepResult = {
        ...
        metadata: {
          parser: filesToProcess.length > 0 ? this.determineParser(step.parser, filesToProcess[0]) : (step.parser || 'auto'),
  ...
  ): Promise<{ content: string; modified: boolean; transformations: Array<any> }> {
   ...
   */
  private determineParser(specifiedParser: string | undefined, filePath: string): 'typescript' | 'javascript' | 'json' | 'text' {
