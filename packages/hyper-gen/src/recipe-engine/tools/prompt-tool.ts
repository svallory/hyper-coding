...
export class PromptTool extends Tool<PromptStep> {
  ...
  protected async onValidate(step: PromptStep, context: StepContext): Promise<ToolValidationResult> {
    const errors: string[] = []
  ...
  protected async onExecute(
    ...
    try {
      ...
      const loggerAdapter = context.logger ? { log: (msg: string) => context.logger?.info(msg) } : undefined
  ...
  private mapPromptType(type: string): string {
    switch (type) {
      ...
      case 'multiselect': return 'multiselect' // assuming supported by prompt tool, otherwise list
