...
export class AiVariableResolver {
  ...
  buildPrompt(
    ...
    for (const { name, config, defaultValue } of unresolvedVars) {
      ...
      if (config.suggestion !== undefined) lines.push(`- **Suggestion:** \`${JSON.stringify(config.suggestion)}\``)
      if (defaultValue !== undefined) lines.push(`- **Default (skipped):** \`${JSON.stringify(defaultValue)}\``)
      if (config.values) lines.push(`- **Allowed values:** ${config.values.map(v => `\`${v}\``).join(', ')}`)
