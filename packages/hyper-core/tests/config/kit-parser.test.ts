...
`.trim(),
      ...
      expect(result.config.defaults).toEqual({ cookbook: 'component', recipe: 'add' })
...
`,
  ...
  describe('getDefaultKitSearchDirs', () => {
    it('returns .hyper/kits, kits/, and node_modules/@hyper-kits/ relative to project root', () => {
      ...
      expect(dirs[2]).toBe(path.join('/my/project', 'node_modules', '@hyper-kits'))
    })
    ...
    it('works with relative-looking project root', () => {
      ...
      expect(dirs[2]).toBe(path.join('.', 'node_modules', '@hyper-kits'))
    })
  })
