...
export class DefaultPluginManager implements PluginManager {
	...
	async discover(options: PluginDiscoveryOptions = {}): Promise<PluginInfo[]> {
		...
		if (includeNodeModules) {
			const nodeModulesPlugins = await this.searchNodeModules(
				baseDir,
				pattern,
				types,
			);
		...
		if (includeLocal) {
			const localPlugins = await this.searchLocalDirectories(
				baseDir,
				pattern,
				types,
			);
	...
	async getPluginInfo(pluginName: string): Promise<PluginInfo | null> {
		const cached = this.infoCache.get(pluginName);
		if (cached) {
			return cached;
		}
		...
		try {
			const packageJsonPath = path.join(
				path.dirname(require.resolve(pluginName)),
				'package.json',
			);
	...
	register(plugin: PluginModule, options: Record<string, any> = {}): void {
		if (plugin.type === 'template-engine' && plugin.createTemplateEngine) {
			...
			} = require('../template-engines/factory.js');
	...
	private async searchNodeModules(
		baseDir: string,
		pattern: string,
		types: string[],
	): Promise<PluginInfo[]> {
		...
		try {
			const packagePaths = await glob(
				path.join(nodeModulesPath, pattern, 'package.json'),
			);
	...
	private async searchLocalDirectories(
		baseDir: string,
		pattern: string,
		types: string[],
	): Promise<PluginInfo[]> {
		...
		while (currentDir !== root) {
			try {
				const packagePaths = await glob(
					path.join(currentDir, pattern, 'package.json'),
				);
	...
	private validatePlugin(
		...
		if (pluginModule.default?.type && pluginModule.default.name) {
			return {
				type: pluginModule.default.type,
				name: pluginModule.default.name,
				createTemplateEngine:
					pluginModule.default.createTemplateEngine ||
					pluginModule.createTemplateEngine,
			};
		}
