...
import { type FilterImplOptions, Liquid, type Tag } from 'liquidjs';
...
export class LiquidTemplateEngine implements TemplateEngine {
	...
	async renderFile(
		...
		try {
			// assuming returned value will ALWAYS be a string
			// see: https://github.com/harttle/liquidjs/issues/832
			return await (this.liquid.renderFile(
				filePath,
				context,
			) as unknown as Promise<string>);
		} catch (error) {
			throw new Error(
				`LiquidJS file rendering failed (${filePath}): ${error.message}`,
			);
		}
	}
	...
	private setupDefaultFilters(): void {
		...
		this.liquid.registerFilter('camelCase', (str: string) =>
			str.replace(/[\s_-]+(.)?/g, (_, char: string) =>
				char ? char.toUpperCase() : '',
			),
		);

		this.liquid.registerFilter('pascalCase', (str: string) => {
			const camelCased = str.replace(/[\s_-]+(.)?/g, (_, char: string) =>
				char ? char.toUpperCase() : '',
			);
