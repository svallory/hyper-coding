...
export abstract class Tool<TStep extends RecipeStepUnion = RecipeStepUnion> {
  ...
  async validate(step: TStep, context: StepContext): Promise<ToolValidationResult> {
    ...
    try {
      ...
      this.debug('Validation completed: %s (errors: %d, warnings: %d)', 
   ...
   */
  async execute(step: TStep, context: StepContext, options?: StepExecutionOptions): Promise<StepResult> {
    if (!this._isInitialized) {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          lastError = error
          this.debug('Tool execution failed on attempt %d: %s', attempt + 1, 
   ...
   */
  private addLifecycleEvent(phase: ToolPhase, success: boolean, metadata?: Record<string, any>): void {
