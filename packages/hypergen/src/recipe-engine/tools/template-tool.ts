...
export class TemplateTool extends Tool<TemplateStep> {
	...
	protected async onInitialize(): Promise<void> {
		...
		try {
			await initializeTemplateEnginesWithPlugins(
				this.options.templateEngineConfig,
			);
	...
	protected async onValidate(
		...
		if (step.engine && !['ejs', 'liquid', 'auto'].includes(step.engine)) {
			errors.push(
				`Invalid template engine: ${step.engine}. Must be 'ejs', 'liquid', or 'auto'`,
			);
		}
	...
	private async resolveTemplate(
		...
		const resolutionPaths = [
			...
			context.templatePath
				? path.resolve(path.dirname(context.templatePath), templateId)
				: null,
