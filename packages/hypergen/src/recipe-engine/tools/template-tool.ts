...
export class TemplateTool extends Tool<TemplateStep> {
  ...
  protected async onValidate(step: TemplateStep, context: StepContext): Promise<ToolValidationResult> {
    ...
    if (step.templateConfig?.composition?.includes?.length && step.templateConfig.composition.includes.length > 10) {
    ...
    try {
      ...
      return {
        ...
        output: {
          ...
          skippedReasons: skippedFiles.map(f => ({ path: f.templatePath, reason: f.skipReason }))
        },
        metadata: {
          templateResolution: {
   ...
   */
  private async resolveTemplate(templateId: string, context: StepContext): Promise<TemplateResolution> {
    // Check cache first
  ...
  private async renderTemplateFile(
    ...
    const { attributes, body } = (fm as any)(templateContent, { allowUnsafe: true })
    ...
    const skipReason = shouldSkip ? this.getSkipReason(renderedAttributes, renderContext) : undefined
  ...
  private async generateFile(
    ...
    const runnerConfig: RunnerConfig = {
      ...
      createPrompter: () => ({ prompt: async (arg: any): Promise<any> => ({ overwrite: step.overwrite || false }) })
    }
