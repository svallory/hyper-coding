	...
	protected async onExecute(
		...
		try {
			...
			for (let i = 0; i < recipe.steps.length; i++) {
				...
				this.debug(
					'Executing sub-recipe step %d: %s (%s)',
					i + 1,
					subStep.name,
					subStep.tool,
				);
				...
				try {
					...
					if (
						subStep.when &&
						!stepContext.evaluateCondition(subStep.when, stepContext.variables)
					) {
					...
					const stepResult = await this.executeSubStep(
						subStep,
						stepContext,
						options,
					);
					...
					this.logger.warn(
						`Sub-recipe step '${subStep.name}' failed but continuing: ${error instanceof Error ? error.message : String(error)}`,
					);
					...
					stepResults.push({
						...
						error: {
							...
							code:
								error instanceof HypergenError ? error.code : 'SUB_STEP_ERROR',
