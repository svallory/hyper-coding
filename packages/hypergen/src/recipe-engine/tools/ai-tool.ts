...
export class AiTool extends Tool<AIStep> {
  ...
  protected async onValidate(step: AIStep, context: StepContext): Promise<ToolValidationResult> {
    ...
    if (!step.output) {
      ...
      if (!validTypes.includes(step.output.type)) {
        errors.push(`Invalid output type "${step.output.type}". Must be one of: ${validTypes.join(', ')}`)
      }
    ...
    if (!hasApiKey) {
      warnings.push('No AI provider API key found in environment. Set ANTHROPIC_API_KEY or OPENAI_API_KEY.')
    }
  ...
  protected async onExecute(
    ...
    try {
      ...
      return {
        ...
        filesModified: step.output.type === 'inject' && step.output.injectInto ? [step.output.injectInto] : undefined,
        output: {
          text: result.output,
   ...
   */
  private async handleOutput(step: AIStep, context: StepContext, result: AIExecutionResult): Promise<void> {
