...
import { HypergenError, ErrorCode, ErrorHandler } from '../../errors/hypergen-errors.js'
...
export class ToolRegistry {
  ...
  register(
    ...
    try {
      ...
      this.debug('Factory validation failed for %s: %s', registrationKey, 
  ...
  async resolve(
    ...
    if (this.enableInstanceReuse && !options?.forceNew) {
      ...
      if (cached && !cached.inUse) {
        ...
        this.debug('Resolved cached tool instance: %s (access count: %d)', 
          registrationKey, cached.accessCount)
    ...
    try {
      ...
      this.debug('Failed to create tool instance: %s - %s', registrationKey,
        error instanceof Error ? error.message : String(error))
  ...
  getStats(): ToolRegistryStats {
    ...
    for (const registration of this.registrations.values()) {
      ...
      if (!stats.byType[type]) {
        stats.byType[type] = { registrations: 0, cachedInstances: 0, activeInstances: 0 }
      }
    ...
    for (const [key, cached] of this.instanceCache) {
      if (cached.instance.isInitialized() && !cached.instance.isCleanedUp()) {
        cached.instance.cleanup().catch(error => {
          this.debug('Error cleaning up cached instance %s: %s', key,
            error instanceof Error ? error.message : String(error))
        })
      }
    }
  ...
  cleanupExpiredInstances(): number {
    ...
    for (const [key, cached] of this.instanceCache) {
      ...
      if (!cached.inUse && age > this.cacheTimeoutMs) {
        ...
        if (cached.instance.isInitialized() && !cached.instance.isCleanedUp()) {
          cached.instance.cleanup().catch(error => {
            this.debug('Error cleaning up expired instance %s: %s', key,
              error instanceof Error ? error.message : String(error))
          })
        }
  ...
  private evictOldestCachedInstance(): void {
    ...
    if (oldestKey) {
      ...
      if (cached.instance.isInitialized() && !cached.instance.isCleanedUp()) {
        cached.instance.cleanup().catch(error => {
          this.debug('Error cleaning up evicted instance %s: %s', oldestKey,
            error instanceof Error ? error.message : String(error))
        })
      }
  ...
  private removeCachedInstances(toolType: ToolType, name: string): void {
    ...
    if (cached) {
      ...
      if (cached.instance.isInitialized() && !cached.instance.isCleanedUp()) {
        cached.instance.cleanup().catch(error => {
          this.debug('Error cleaning up removed instance %s: %s', registrationKey,
            error instanceof Error ? error.message : String(error))
        })
      }
