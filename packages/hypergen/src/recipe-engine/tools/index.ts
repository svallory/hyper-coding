...
import type { RecipeStepUnion, ToolType } from '../types.js';
 ...
 */
export const SUPPORTED_TOOL_TYPES: readonly ToolType[] = [
	'template',
	'action',
	'codemod',
	'recipe',
...
export function validateToolType(type: string): ToolType {
	if (!isSupportedToolType(type)) {
		throw new Error(
			`Unsupported tool type: ${type}. Supported types: ${SUPPORTED_TOOL_TYPES.join(', ')}`,
		);
	}
	return type;
}
