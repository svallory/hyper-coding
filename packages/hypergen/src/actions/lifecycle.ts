	...
	private async executeHooks(
		...
		for (const hook of phaseHooks) {
			...
			try {
				...
				debug(
					'Hook %s completed successfully in %dms',
					hook.name,
					hookResult.duration,
				);
			} catch (error: any) {
...
export class BuiltinHooks {
	...
	static installDependencies: LifecycleHook = {
		...
		condition: (context: ActionContext) => {
			...
			return (
				context.variables.installDependencies === true ||
				(context.variables.filesCreated || []).some((file: string) =>
	...
	static getAll(): LifecycleHook[] {
		return [
			BuiltinHooks.formatFiles,
			BuiltinHooks.installDependencies,
			BuiltinHooks.initGit,
			BuiltinHooks.backupFiles,
			BuiltinHooks.validatePrerequisites,
		];
	}
}
