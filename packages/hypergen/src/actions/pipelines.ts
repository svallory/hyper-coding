...
export class ActionPipelineManager {
	...
	registerPipeline(config: PipelineConfig): void {
		...
		debug(
			'Pipeline registered: %s with %d steps',
			config.name,
			config.steps.length,
		);
	}
	...
	async executePipeline(
		...
		try {
			...
			execution.status =
				execution.metadata.failedSteps > 0 ? 'failed' : 'completed';
			execution.endTime = new Date();
			execution.duration =
				execution.endTime.getTime() - execution.startTime.getTime();

			debug(
				'Pipeline execution completed: %s [%s]',
				pipelineName,
				execution.status,
			);
			return execution;
	...
	private async executeSteps(
		...
		const stepExecutionMap = new Map(
			execution.steps.map((step) => [step.stepId, step]),
		);
	...
	private async executeStep(
		...
		const stepContext: Partial<ActionContext> = {
			...
			communication: {
				...
				setSharedData: (key: string, value: any) => {
					this.communicationManager.setSharedData(
						key,
						value,
						stepExecution.actionId,
					);
		...
		throw (
			lastError ||
			new Error(`Step ${step.id} failed after ${maxRetries} retries`)
		);
	}
	...
	private getReadySteps(
		...
		return steps.filter((step) => {
			...
			if (!execution) {
				return false; // Skip steps without execution state
			}
		...
		for (const step of execution.steps) {
			if (step.status === 'running') {
				...
				step.duration =
					...
					(step.startTime?.getTime() || execution.startTime.getTime());
			}
		}
