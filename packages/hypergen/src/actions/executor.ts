...
export class ActionExecutor {
	...
	async executeInteractively(
		...
		try {
			...
			const { result, lifecycle } =
				await this.lifecycleManager.executeLifecycle(
					actionName,
					executionContext,
					mainAction,
					{
	...
	registerLifecycleHook(
		actionName: string,
		hook: import('./lifecycle.js').LifecycleHook,
	): void {
	...
	getLifecycleHooks(
		actionName: string,
	): import('./lifecycle.js').LifecycleHook[] {
	...
	async executeWorkflow(
		...
		for (const actionConfig of actions) {
			...
			if (actionConfig.dependsOn) {
				this.communicationManager.setSharedData(
					`${actionId}_dependencies`,
					actionConfig.dependsOn,
					'workflow',
				);
			}
		}
		...
		for (const actionConfig of actions) {
			...
			if (!actionId) {
				throw new Error(`Action ID not found for action: ${actionConfig.name}`);
			}
