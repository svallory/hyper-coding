...
export class CostTracker {
   ...
   */
  record(stepName: string, model: string, inputTokens: number, outputTokens: number, retryAttempts: number = 0): void {
    const costUsd = this.calculateCost(model, inputTokens, outputTokens)

    this.steps.push({ stepName, model, inputTokens, outputTokens, costUsd, retryAttempts })
  ...
  checkBudget(): void {
    ...
    // Check token limit
    if (this.budget.maxTotalTokens && (this.totalInputTokens + this.totalOutputTokens) >= this.budget.maxTotalTokens) {
    ...
    if (this.budget.warnAtCostUsd && this.totalCostUsd >= this.budget.warnAtCostUsd && !this.budgetWarningTriggered) {
