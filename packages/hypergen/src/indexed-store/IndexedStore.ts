import type { IndexedStoreItemType, PrimitiveType, ToTuple } from './types.js';
...
export abstract class BaseIndexedStore<
	...
	KeyFunction extends (...args: PrimitiveType[]) => KeyType = (
		...args: ToTuple<ItemType, KeyParts> & PrimitiveType[]
	) => KeyType,
> {
	protected items: Map<KeyType, ItemType> = new Map();

	protected indices: Map<
		keyof ItemType,
		Map<ItemType[keyof ItemType], ItemType[]>
	> = new Map();
	...
	protected indexItem(
		item: ItemType,
		propName: keyof ItemType,
		index: Map<any, ItemType[]>,
	) {
...
`);
	...
	exec(
		callerName: keyof BaseIndexedStore<
			ItemType,
			KeyParts,
			ExtraIndices,
			KeyType,
		...
		>,
		fn: (key: KeyType) => any,
		...itemOrKeyOrParts: any[]
	): any {
		if (!itemOrKeyOrParts?.[0]) {
			throw new Error(
				`Invalid parameter (${JSON.stringify(itemOrKeyOrParts[0])} receive in IndexedStore.${callerName}`,
			);
		}
...
`);
	...
	private removeByKey(key: KeyType): ItemType | undefined {
		...
		for (const [propName, index] of this.indices) {
			...
			if (propValue !== undefined) {
				const itemsAtIndex = index.get(propValue);
				if (itemsAtIndex) {
					const filteredItems = itemsAtIndex.filter(
						(i) => this.keyFor(i) !== key,
					);
					if (filteredItems.length > 0) {
						index.set(propValue, filteredItems);
					} else {
						index.delete(propValue);
					}
				}
			}
		}
