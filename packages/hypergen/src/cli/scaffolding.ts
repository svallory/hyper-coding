...
`;
	 ...
	 */
	private generateExampleTemplates(
		options: ScaffoldingOptions,
		...
		switch (framework) {
			case 'react':
				templates['templates/component.tsx.ejs'] =
					this.getReactComponentTemplate();
				templates['templates/component.test.tsx.ejs'] =
					this.getReactTestTemplate();
				templates['templates/component.stories.tsx.ejs'] =
					this.getReactStoryTemplate();
				break;

			case 'vue':
				templates['templates/component.vue.ejs'] =
					this.getVueComponentTemplate();
				templates['templates/component.test.ts.ejs'] =
					this.getVueTestTemplate();
				break;
...
${this.getFrameworkParameters(options.framework || 'generic')
	.map(
		(p) =>
...
# With all options
hypergen action ${options.name} --name=my-${options.name} ${this.getFrameworkParameters(
...
${this.getFrameworkFileList(options.framework || 'generic')
	.map((f) => `- ${f}`)
	.join('\n')}
...
`;
	...
	private getFrameworkParameters(framework: string): Array<{
		name: string;
		type: string;
		required?: boolean;
		default?: any;
				...
				return [
					{
						name: 'type',
						type: 'enum',
						values: ['sfc', 'composition', 'options'],
						default: 'composition',
						description: 'Component type',
					},
					{
					...
					},
					{
						...
						type: 'boolean',
						default: false,
						description: 'Include emits definition',
					},
				];
			case 'api':
				return [
				...
				return [
					{
						...
						type: 'boolean',
						default: true,
						description: 'Include command options',
					},
					{
						name: 'hasSubcommands',
						type: 'boolean',
						default: false,
						description: 'Include subcommands',
					},
				];
			case 'node':
				return [
	...
	private getFrameworkImplementation(
		framework: string,
		options: ScaffoldingOptions,
	): string {
	...
	private getFrameworkHelperFunctions(
		framework: string,
		options: ScaffoldingOptions,
	): string {
		const pascalName = this.toPascalCase(options.name);
