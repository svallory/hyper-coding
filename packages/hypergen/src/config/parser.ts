...
export class TemplateConfigParser {
	...
	async parseConfig(configPath: string): Promise<ParsedTemplateConfig> {
		...
		try {
			...
			throw new TemplateConfigError(`Failed to read config file: ${error.message}`, configPath);
		}
	}
	...
	async parseConfigFromString(
		...
		try {
			...
			throw new TemplateConfigError(`YAML parsing failed: ${error.message}`, configPath);
		}
	}
