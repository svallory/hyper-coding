...
export class TemplateConfigParser {
	...
	async parseConfig(configPath: string): Promise<ParsedTemplateConfig> {
		...
		try {
			...
			return await this.parseConfigFromString(
				configContent,
				basePath,
				configPath,
			);
	...
	validateConfig(config: any): { valid: boolean; errors?: ValidationError[] } {
		...
		if (config.variables) {
			if (typeof config.variables !== 'object') {
				...
				for (const [name, definition] of Object.entries(config.variables)) {
					const varErrors = this.validateVariableDefinition(
						name,
						definition as any,
					);
		...
		if (config.includes) {
			if (!Array.isArray(config.includes)) {
				...
				for (const [index, include] of config.includes.entries()) {
					if (!include.url) {
						errors.push({
	...
	async resolveVariables(
		...
		for (const [name, definition] of Object.entries(variables)) {
			...
			// Validate the value
			if (resolved.values[name] !== undefined) {
				const validation = this.validateVariableValue(
					name,
	...
	private validateVariableValue(
				...
				if (definition.pattern) {
					...
					if (!regex.test(value)) {
						return {
							valid: false,
							message:
								definition.validation?.message ||
								`Value does not match pattern: ${definition.pattern}`,
						};
					}
				}
				break;
		...
		if (definition.validation?.custom) {
			const validator =
				this.options.customValidators[definition.validation.custom];
			if (validator) {
	...
	private async resolveIncludes(
		...
		for (const include of config.includes) {
			...
			try {
				...
				// Check cache first
				const cachedInclude = this.options.includeCache.get(include.url);
				if (cachedInclude) {
					includeConfig = cachedInclude;
					debug('Found cached include: %s', include.url);
				} else {
					// Resolve include URL
					includeConfig = await this.resolveIncludeUrl(
						include.url,
						config.basePath,
					);
	...
	private async resolveIncludeUrl(
		url: string,
		basePath: string,
	): Promise<ParsedTemplateConfig> {
		if (
			!this.options.allowRemoteIncludes &&
			!url.startsWith('./') &&
			!url.startsWith('../') &&
			!path.isAbsolute(url)
		) {
	...
	private mergeVariables(
		target: ParsedTemplateConfig,
		source: ParsedTemplateConfig,
	): void {
		...
		for (const [name, definition] of Object.entries(source.allVariables)) {
			if (target.allVariables[name]) {
				...
				switch (this.options.conflictResolution) {
					case 'fail':
						throw new Error(
							`Variable conflict: '${name}' is defined in multiple templates`,
						);
					case 'override':
