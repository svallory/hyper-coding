...
export function registerCookbookHelpers(cookbook: ParsedCookbook): void {
	if (cookbook.loadedHelpers && Object.keys(cookbook.loadedHelpers).length > 0) {
		...
		debug(
			"Registered %d helpers for cookbook: %s",
			Object.keys(cookbook.loadedHelpers).length,
			cookbook.config.name,
		);
	}
}
...
function validateCookbookConfig(parsed: any, errors: string[], warnings: string[]): CookbookConfig {
