...
export class TemplateCompositionEngine {
					...
					switch (conflict.resolution) {
						...
						case 'error':
							throw ErrorHandler.createError(
								...
								{
									variable: name,
									extends: context.baseTemplate.extends,
									template: context.baseTemplate.name,
								},
							);
					}
				} else {
				...
				switch (conflict.resolution) {
					...
					case 'error':
						throw ErrorHandler.createError(
							...
							{
								variable: name,
								base: context.baseTemplate.name,
								include: include.url,
							},
						);
				}
	...
	private async resolveTemplate(
		...
		const cached = context.resolvedTemplates.get(cacheKey);
		if (cached) {
			return cached;
		}
	...
	private isSimpleVariableCheck(expression: string): boolean {
		...
		return /^[a-zA-Z_][a-zA-Z0-9_]*(\s*[!=]==?\s*(true|false|null|undefined|'[^']*'|"[^"]*"|\d+))?$/.test(
			expression,
		);
	}
