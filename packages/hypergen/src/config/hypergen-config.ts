...
/**
 * Type for helper functions that can be used in templates
 * Each helper is a function that can accept any arguments and return any value
 */
export type HelperFunction = (...args: unknown[]) => unknown;

/**
 * Collection of helper functions indexed by name
 */
export type HelperFunctions = Record<string, HelperFunction>;

/**
 * Factory function that creates helpers based on locals and config
 */
export type HelperFactory = (locals: any, config: any) => HelperFunctions;
...
export interface HypergenConfig {
	...
	// Custom helpers
	helpers?: string | HelperFunctions | HelperFactory;
...
export class HypergenConfigLoader {
	...
	private static readonly DEFAULT_CONFIG: Required<HypergenConfig> = {
		templates: ['recipes'],
		discovery: {
		...
		try {
			...
			if (configPath) {
				if (fs.existsSync(configPath)) {
					config = await HypergenConfigLoader.loadConfigFile(configPath);
			...
			if (!actualConfigPath) {
				...
				if (envConfigPath && fs.existsSync(envConfigPath)) {
					config = await HypergenConfigLoader.loadConfigFile(envConfigPath);
			...
			if (!actualConfigPath) {
				for (const fileName of HypergenConfigLoader.CONFIG_FILES) {
					const fullPath = path.join(projectRoot, fileName);
					if (fs.existsSync(fullPath)) {
						config = await HypergenConfigLoader.loadConfigFile(fullPath);
			...
			// Load helpers
			const loadedHelpers = await HypergenConfigLoader.loadHelpers(
			...
			const resolvedConfig: ResolvedConfig = {
				...
				templates:
					...
					HypergenConfigLoader.DEFAULT_CONFIG.templates,
				discovery:
					mergedConfig.discovery ||
					HypergenConfigLoader.DEFAULT_CONFIG.discovery,
				engine:
					mergedConfig.engine || HypergenConfigLoader.DEFAULT_CONFIG.engine,
				output:
					mergedConfig.output || HypergenConfigLoader.DEFAULT_CONFIG.output,
				validation:
					mergedConfig.validation ||
					HypergenConfigLoader.DEFAULT_CONFIG.validation,
				cache:
					mergedConfig.cache || HypergenConfigLoader.DEFAULT_CONFIG.cache,
				plugins:
					mergedConfig.plugins ||
					HypergenConfigLoader.DEFAULT_CONFIG.plugins,
				helpers:
					mergedConfig.helpers ||
					HypergenConfigLoader.DEFAULT_CONFIG.helpers,
				environments:
					mergedConfig.environments ||
					HypergenConfigLoader.DEFAULT_CONFIG.environments,
			};
	...
	private static async loadHelpers(
		helpers: string | HelperFunctions | HelperFactory | undefined,
		projectRoot: string,
	): Promise<HelperFunctions> {
		if (!helpers) return {};
