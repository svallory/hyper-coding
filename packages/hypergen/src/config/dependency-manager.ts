...
export class TemplateDependencyManager {
	...
	async resolveDependencies(
		...
		debug(
			'Dependency resolution complete: %d resolved, %d conflicts, %d missing',
	...
	private async resolveSingleDependency(
		...
		try {
			...
			debug(
				'Optional dependency %s failed to resolve: %s',
				dependency.name,
				error.message,
			);
		}
	...
	private async resolveAutoDependency(
		...
		const strategies = [
			...
			() =>
				this.resolveGitHubDependency(
					{ ...dependency, type: 'github' },
					options,
				),
		];
	...
	private detectConflicts(dependencies: ResolvedDependency[]): Array<{
		...
		for (const [name, deps] of dependencyMap) {
			if (deps.length > 1) {
				const versions = [
					...new Set(deps.map((d) => d.version).filter(Boolean)),
				];
