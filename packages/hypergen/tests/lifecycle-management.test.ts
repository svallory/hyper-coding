...
describe('Action Lifecycle Management', () => {
	...
	describe('Lifecycle Execution', () => {
		...
		it('should execute error hooks when main action fails', async () => {
			...
			await expect(
				lifecycleManager.executeLifecycle(
					'failing-action',
					mockContext,
					failingAction,
				),
	...
	describe('Built-in Hooks', () => {
		it('should provide built-in hooks', () => {
			...
			expect(builtinHooks.some((h) => h.name === 'install-dependencies')).toBe(
				true,
			);
		...
		it('should execute install-dependencies hook conditionally', async () => {
			...
			const result1 =
				await BuiltinHooks.installDependencies.action(mockContext);
			...
			const result2 =
				await BuiltinHooks.installDependencies.action(contextWithFlag);
		...
		it('should execute validate prerequisites hook', async () => {
			const result =
				await BuiltinHooks.validatePrerequisites.action(mockContext);
	...
	describe('Hook Management', () => {
		it('should clear all hooks', () => {
			...
			lifecycleManager.registerHook('test-action', testHook);
			expect(lifecycleManager.getHooks('test-action').length).toBeGreaterThan(
				0,
			);
		...
		it('should clear hooks for specific action', () => {
			...
			expect(
				lifecycleManager
					.getHooks('action1')
					.filter((h) => h.name === 'test-hook-1'),
			).toHaveLength(0);
			expect(
				lifecycleManager
					.getHooks('action2')
					.filter((h) => h.name === 'test-hook-2'),
			).toHaveLength(1);
		});
	});
