...
describe('V8 Integration Tests', () => {
	...
	describe('Template.yml Configuration System', () => {
		...
		it('should list templates', async () => {
			...
			expect(stdout).toContain(
				'• my-test-generator - Test generator for React components',
			);
		});
	...
	describe('End-to-End Action Execution', () => {
		it('should execute action and generate files', async () => {
			...
			expect(stdout).toContain(
				'✅ Created my-test-generator TestComponent with 2 files',
			);
			expect(stdout).toContain(
				"✅ Action 'my-test-generator' completed successfully",
			);
			...
			expect(mainContent).toContain(
				'// Generated my-test-generator: TestComponent',
			);
		...
		it('should handle missing action gracefully', async () => {
			try {
				...
				expect(error.stdout).toContain(
					"❌ Action 'non-existent-action' not found",
				);
	...
	describe('Recipe Step System - Tools Framework', () => {
		...
		it('should maintain tool registry state', async () => {
			const { getToolRegistry, ToolRegistry } = await import(
				'../src/recipe-engine/tools/index.js'
			);
		...
		it('should support all required tool types', async () => {
			const { SUPPORTED_TOOL_TYPES } = await import(
				'../src/recipe-engine/tools/index.js'
			);
		...
		it('should check registry health', async () => {
			const { checkRegistryHealth, ToolRegistry } = await import(
				'../src/recipe-engine/tools/index.js'
			);
