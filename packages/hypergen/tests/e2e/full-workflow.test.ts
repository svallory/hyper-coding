...
const KIT_PATH = path.resolve(__dirname, '../../kits/nextjs')
...
async function executeResourceTemplates(
  ...
  const templates = ['schema.ts.jig', 'actions.ts.jig', 'form-component.tsx.jig']
  ...
  for (const template of templates) {
    ...
    if (await fs.pathExists(templatePath)) {
      ...
      const recipePath = path.join(dir, `_mini_${name}_${template.replace(/\./g, '_')}.yml`)
...
async function executePageTemplate(
  ...
  const templatePath = path.join(KIT_PATH, 'cookbooks/page/add/templates/page.tsx.jig')
  ...
  const recipePath = path.join(dir, `_page_${pagePath.replace(/\//g, '_')}.yml`)
  await fs.writeFile(recipePath, yaml.dump(recipeObj))
 ...
 */
const buildFieldDescriptions = (modelFields: Array<{ name: string; type: string; isRequired?: boolean; documentation?: string }>) => {
  return modelFields.map((f: any) =>
...
describe('Full Workflow: Multi-tenant Todo App E2E', () => {
  ...
  describe('Pass 1: AI Block Collection', () => {
    ...
    it('should not create files during Pass 1', async () => {
      ...
      expect(await fs.pathExists(path.join(tempDir, 'lib/schemas/organization-schema.ts'))).toBe(false)
      expect(await fs.pathExists(path.join(tempDir, 'app/actions/organization.ts'))).toBe(false)
    })
  })
  ...
  describe('Pass 2: File Generation with Fixture Answers', () => {
    it('should generate organization schema with Zod validation', async () => {
      ...
      const schemaPath = path.join(tempDir, 'lib/schemas/organization-schema.ts')
    ...
    it('should generate all three models without conflicts', async () => {
      ...
      // All schemas exist
      expect(await fs.pathExists(path.join(tempDir, 'lib/schemas/organization-schema.ts'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'lib/schemas/member-schema.ts'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'lib/schemas/todo-schema.ts'))).toBe(true)

      // All actions exist
      expect(await fs.pathExists(path.join(tempDir, 'app/actions/organization.ts'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'app/actions/member.ts'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'app/actions/todo.ts'))).toBe(true)

      // All forms exist
      expect(await fs.pathExists(path.join(tempDir, 'components/OrganizationForm.tsx'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'components/MemberForm.tsx'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'components/TodoForm.tsx'))).toBe(true)
    })
  ...
  describe('Page Generation', () => {
    it('should generate static pages (login, signup, dashboard)', async () => {
      await executePageTemplate(engine, tempDir, 'login', 'Login to your account')
      await executePageTemplate(engine, tempDir, 'signup', 'Create a new account')
      await executePageTemplate(engine, tempDir, 'dashboard', 'Your dashboard overview')

      // All pages exist
      expect(await fs.pathExists(path.join(tempDir, 'app/login/page.tsx'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'app/signup/page.tsx'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'app/dashboard/page.tsx'))).toBe(true)

      // Login page content
      const loginContent = await fs.readFile(path.join(tempDir, 'app/login/page.tsx'), 'utf-8')
      ...
      const dashboardContent = await fs.readFile(path.join(tempDir, 'app/dashboard/page.tsx'), 'utf-8')
  ...
  describe('Content Quality', () => {
    ...
    it('should produce valid TypeScript imports', async () => {
      ...
      const form = await fs.readFile(
        path.join(tempDir, 'components/OrganizationForm.tsx'), 'utf-8',
      )
    ...
    it('should maintain cross-file consistency', async () => {
      ...
      const schema = await fs.readFile(
        path.join(tempDir, 'lib/schemas/todo-schema.ts'), 'utf-8',
      )
      const actions = await fs.readFile(
        path.join(tempDir, 'app/actions/todo.ts'), 'utf-8',
      )
      const form = await fs.readFile(
        path.join(tempDir, 'components/TodoForm.tsx'), 'utf-8',
      )
