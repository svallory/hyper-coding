...
function validateGoSyntax(filePath: string): { valid: boolean; error?: string } {
  try {
...
function validateGoStructure(content: string, expectedElements: {
  package?: string
  imports?: string[]
  types?: string[]
  functions?: string[]
...
describe('Generated File Content E2E', () => {
  ...
  describe('Route Injection', () => {
    ...
    it('should inject routes without creating duplicates', async () => {
      ...
      const firstGetCount = (firstContent.match(/GET \/app\/organization\/\{id\}\/edit/g) || []).length
      const firstPostCount = (firstContent.match(/POST \/app\/organization\/\{id\}\/edit/g) || []).length
