...
describe('Advanced Composition Integration', () => {
		...
		const processTemplateAction = {
			...
			fn: async (context: ActionContext): Promise<ActionResult> => {
				...
				const template: TemplateConfig = {
					...
					dependencies: variables.enableDeps
						? ['react', 'typescript']
						: undefined,
				};
		...
		const generateCodeAction = {
			...
			fn: async (context: ActionContext): Promise<ActionResult> => {
				...
				const compositionResult =
					context.communication?.getSharedData('compositionResult');
				const dependencyAnalysis =
					context.communication?.getSharedData('dependencyAnalysis');
	...
	describe('Full Integration Workflow', () => {
		it('should execute a complete template-to-code pipeline with all features', async () => {
			...
			const fullPipeline: PipelineConfig = {
				...
				description:
					'Complete pipeline demonstrating all advanced composition features',
				version: '1.0.0',
				variables: {
			...
			const validationStep = execution.steps.find(
				(s) => s.stepId === 'validation',
			)!;
			...
			const compositionResult =
				communicationManager.getSharedData('compositionResult');
			expect(compositionResult).toBeDefined();
			expect(compositionResult.variables).toBeGreaterThan(0);

			const dependencyAnalysis =
				communicationManager.getSharedData('dependencyAnalysis');
		...
		it('should handle conditional execution based on shared data', async () => {
			...
			const conditionalStep = execution.steps.find(
				(s) => s.stepId === 'conditional-step',
			)!;
	...
	describe('Performance and Scalability', () => {
		it('should handle complex workflows efficiently', async () => {
			...
			const execution =
				await pipelineManager.executePipeline('complex-workflow');
	...
	describe('Real-world Scenarios', () => {
		it('should handle a complete React component generation workflow', async () => {
			...
			const finalStep = execution.steps.find(
				(s) => s.stepId === 'generate-component',
			)!;
			...
			expect(finalStep.result?.filesCreated).toContain(
