...
describe('Conditional Template Inclusion', () => {
	...
	describe('Simple Variable Conditions', () => {
		it('should evaluate simple boolean variable conditions', async () => {
			...
			expect(composedTS.resolvedIncludes[1].reason).toContain(
				'Condition not met',
			);
			...
			expect(composedJS.resolvedIncludes[0].reason).toContain(
				'Condition not met',
			);
		...
		it('should evaluate string comparison conditions', async () => {
			...
			expect(composed.resolvedIncludes[1].reason).toContain(
				'Condition not met',
			);
		});
		...
		it('should evaluate number comparison conditions', async () => {
			...
			expect(composed.resolvedIncludes[0].reason).toContain(
				'Condition not met',
			);
	...
	describe('Complex Conditions', () => {
		it('should evaluate logical AND conditions', async () => {
			...
			expect(composedPartial.resolvedIncludes[0].reason).toContain(
				'Condition not met',
			);
		});
		...
		it('should evaluate logical OR conditions', async () => {
			...
			expect(composedNone.resolvedIncludes[0].reason).toContain(
				'Condition not met',
			);
		});
	});
		...
		it('should handle many conditional includes efficiently', async () => {
			...
			for (let i = 0; i < 50; i++) {
				if (i % 2 === 0) {
					...
					expect(composed.resolvedIncludes[i].reason).toContain(
						'Condition not met',
					);
				}
			}
