...
const createMockAction = (name: string, duration = 10, shouldFail = false) => {
	return {
		metadata: {
			name,
			description: `Mock action ${name}`,
			parameters: [{ name: 'input', type: 'string' as const, required: false }],
		},
...
describe('Action Pipelines', () => {
	...
	describe('Pipeline Registration', () => {
		...
		it('should detect invalid dependencies', () => {
			const pipeline: PipelineConfig = {
				name: 'invalid-deps',
				steps: [{ id: 'step1', name: 'setup', dependsOn: ['nonexistent'] }],
			};
	...
	describe('Pipeline Execution', () => {
		it('should execute a simple sequential pipeline', async () => {
			...
			expect(setupStep.startTime?.getTime()).toBeLessThan(
				buildStep.startTime?.getTime(),
			);
			expect(buildStep.startTime?.getTime()).toBeLessThan(
				testStep.startTime?.getTime(),
			);
		});
		...
		it('should execute parallel steps correctly', async () => {
			const pipeline: PipelineConfig = {
				...
				steps: [
					...
					},
					{
						id: 'deploy-step',
						name: 'deploy',
						dependsOn: ['build-step', 'test-step'],
					},
				],
			};
			...
			pipelineManager.registerPipeline(pipeline);
			const execution =
				await pipelineManager.executePipeline('parallel-pipeline');
			...
			const buildStep = execution.steps.find((s) => s.stepId === 'build-step')!;
			const testStep = execution.steps.find((s) => s.stepId === 'test-step')!;
			const deployStep = execution.steps.find(
				(s) => s.stepId === 'deploy-step',
			)!;

			expect(setupStep.endTime?.getTime()).toBeLessThanOrEqual(
				buildStep.startTime?.getTime(),
			);
			expect(setupStep.endTime?.getTime()).toBeLessThanOrEqual(
				testStep.startTime?.getTime(),
			...
			expect(
				Math.max(buildStep.endTime?.getTime(), testStep.endTime?.getTime()),
			).toBeLessThanOrEqual(deployStep.startTime?.getTime());
		});
		...
		it('should handle step failures with continueOnError', async () => {
			...
			pipelineManager.registerPipeline(pipeline);
			const execution =
				await pipelineManager.executePipeline('failure-pipeline');
			...
			const cleanupStep = execution.steps.find(
				(s) => s.stepId === 'cleanup-step',
			)!;
		...
		it('should handle step retries', async () => {
			...
			const pipeline: PipelineConfig = {
				name: 'retry-pipeline',
				steps: [{ id: 'retry-step', name: 'retry-action', retries: 3 }],
			};
	...
	describe('Pipeline Management', () => {
		...
		it('should track pipeline executions', async () => {
			...
			const execution1 =
				await pipelineManager.executePipeline('tracked-pipeline');
			const execution2 =
				await pipelineManager.executePipeline('tracked-pipeline');
			...
			const completedExecutions =
				pipelineManager.getExecutionsByStatus('completed');
	...
	describe('Error Handling', () => {
		...
		it('should handle step timeout gracefully', async () => {
			...
			const execution =
				await pipelineManager.executePipeline('timeout-pipeline');
	...
	describe('Complex Workflow Scenarios', () => {
		...
		it('should handle pipeline with complex dependencies', async () => {
			...
			expect(getStep('a').endTime?.getTime()).toBeLessThanOrEqual(
				getStep('b').startTime?.getTime(),
			);
			expect(getStep('a').endTime?.getTime()).toBeLessThanOrEqual(
				getStep('c').startTime?.getTime(),
			);
			expect(getStep('b').endTime?.getTime()).toBeLessThanOrEqual(
				getStep('d').startTime?.getTime(),
			);
			expect(getStep('c').endTime?.getTime()).toBeLessThanOrEqual(
				getStep('d').startTime?.getTime(),
			);
			expect(getStep('d').endTime?.getTime()).toBeLessThanOrEqual(
				getStep('e').startTime?.getTime(),
			);
		});
	});
});
