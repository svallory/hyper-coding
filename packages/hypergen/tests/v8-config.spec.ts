...
`;
			...
			expect(config.allVariables.framework.values).toEqual([
				'react',
				'vue',
				'angular',
			]);
...
`;
			...
			expect(config.hooks?.before[0].variables).toEqual({
				packages: ['react', '@types/react'],
			});
	...
	describe('validateConfig', () => {
		...
		it('should validate number constraints types', () => {
			...
			expect(result.errors).toBeDefined();
			expect(
				result.errors?.some((e) => e.field === 'variables.count.min'),
			).toBe(true);
			expect(
				result.errors?.some((e) => e.field === 'variables.count.max'),
			).toBe(true);
		});
...
`);
			...
			const resolved = await parser.resolveVariables(config, {
				name: 'TestComponent',
			});
...
`);
			...
			await expect(
				parser.resolveVariables(config, {
					name: 'ValidName',
					count: 5,
					framework: 'react',
				}),
				...
				customValidators: {
					componentName: (value, context) => {
						if (typeof value === 'string' && value.startsWith('Test')) {
							return {
								valid: false,
								message: 'Component name cannot start with Test',
							};
						}
						return { valid: true };
					},
...
`);
			...
			await expect(
				customParser.resolveVariables(config, {
					name: 'TestComponent',
				}),
...
`);
		...
		await expect(
			parser.resolveVariables(config, { name: 123 }),
...
`);
		...
		await expect(
			parser.resolveVariables(config, { enabled: 'true' }),
...
`);
		...
		await expect(
			parser.resolveVariables(config, { count: '42' }),
...
`);
		...
		await expect(
			parser.resolveVariables(config, { items: 'not-array' }),
