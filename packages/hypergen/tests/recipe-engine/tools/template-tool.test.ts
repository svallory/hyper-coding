...
import { TemplateTool, TemplateToolFactory, templateToolFactory } from '../../../src/recipe-engine/tools/template-tool.js'
...
describe('TemplateTool', () => {
  ...
  beforeEach(async () => {
    ...
    stepContext = {
      ...
      stepData: {},
      evaluateCondition: (expression: string) => expression === 'true' || expression === 'typescript'
    } as StepContext
  })
  ...
  describe('execution with real file system', () => {
    ...
    it('should execute with dry run mode successfully', async () => {
      ...
      if (result.status === 'failed') {
        console.log('Test execution failed (expected for integration test):', result.error?.message)
    ...
    it('should handle skip conditions', async () => {
      ...
      const result = await tool.execute(templateStep, { ...stepContext, dryRun: true })
