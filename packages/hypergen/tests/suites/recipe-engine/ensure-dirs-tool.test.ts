...
import { EnsureDirsTool, EnsureDirsToolFactory, ensureDirsToolFactory } from '~/recipe-engine/tools/ensure-dirs-tool'
...
describe('EnsureDirsTool', () => {
  ...
  describe('Validation (via validate())', () => {
    it('should fail if paths field is missing', async () => {
      const tool = new EnsureDirsTool()
      const context = createContext(testDir)
      const step = { tool: 'ensure-dirs', name: 'bad-step' } as unknown as EnsureDirsStep
    ...
    it('should fail if paths is not an array', async () => {
      const tool = new EnsureDirsTool()
      const context = createContext(testDir)
      const step = { tool: 'ensure-dirs', name: 'bad', paths: 'not-an-array' } as any
  ...
  describe('Error Handling', () => {
    ...
    it('should return failed when paths is undefined (runtime crash caught)', async () => {
      const tool = new EnsureDirsTool()
      const context = createContext(testDir)
      const step = { tool: 'ensure-dirs', name: 'bad' } as unknown as EnsureDirsStep
  ...
  describe('Factory', () => {
    ...
    it('should return ToolValidationResult from validateConfig()', () => {
      const factory = new EnsureDirsToolFactory()
      const result: ToolValidationResult = factory.validateConfig({ tool: 'ensure-dirs', paths: ['src'] })
