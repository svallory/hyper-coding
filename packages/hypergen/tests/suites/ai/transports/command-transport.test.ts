...
import type { TransportContext } from '../../../../src/ai/transports/types.js'
...
describe('CommandTransport', () => {
  ...
  function makeContext(collector: AiCollector, command: string, commandMode?: 'batched' | 'per-block'): TransportContext {
    return {
      collector,
  ...
  describe('batched mode', () => {
    it('resolves answers from echo command returning JSON', async () => {
      const collector = makeCollector()
      const jsonResponse = JSON.stringify({ greeting: 'Hello!', farewell: 'Goodbye!' })
