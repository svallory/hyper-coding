...
describe('E2E: edit-page recipe with 2-pass AI generation', () => {
  ...
  describe('Pass 1: Collect Mode', () => {
    ...
    it('should NOT create any output files', async () => {
      ...
      expect(await fs.pathExists(path.join(tempDir, 'internal/handler/organization_edit.go'))).toBe(false)
      // No templ page should exist
      expect(await fs.pathExists(path.join(tempDir, 'internal/ui/pages/app_organization_edit.templ'))).toBe(false)
  ...
  describe('Pass 2: Resolve with Answers', () => {
    ...
    it('should preserve non-AI static template content', async () => {
      ...
      expect(handler).toContain('func (h *OrganizationEditHandler) EditPage(w http.ResponseWriter, r *http.Request)')
      expect(handler).toContain('func (h *OrganizationEditHandler) Update(w http.ResponseWriter, r *http.Request)')
  ...
  describe('Full Round-Trip', () => {
    it('should complete Pass 1 → Assemble → Pass 2 end-to-end', async () => {
      ...
      expect(await fs.pathExists(path.join(tempDir, 'internal/handler/organization_edit.go'))).toBe(true)
      expect(await fs.pathExists(path.join(tempDir, 'internal/ui/pages/app_organization_edit.templ'))).toBe(true)

      // Verify handler content
      const handler = await fs.readFile(path.join(tempDir, 'internal/handler/organization_edit.go'), 'utf8')
      ...
      const templ = await fs.readFile(path.join(tempDir, 'internal/ui/pages/app_organization_edit.templ'), 'utf8')
