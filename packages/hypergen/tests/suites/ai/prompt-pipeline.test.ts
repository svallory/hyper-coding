...
import { PromptPipeline } from '../../../src/ai/prompt-pipeline.js'
import type { StepResult } from '../../../src/recipe-engine/types.js'
  ...
  describe('assemble', () => {
    ...
    it('includes few-shot examples', async () => {
      const result = await pipeline.assemble({
        ...
        examples: [
          { label: 'Example 1', input: 'Create a User class', output: 'class User {}' },
        ],
        projectRoot: tmpDir,
        stepResults: new Map(),
      })
