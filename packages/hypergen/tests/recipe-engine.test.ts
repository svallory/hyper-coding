...
`
      ...
      expect(result.validation.errors.some(e => e.code === 'MISSING_NAME')).toBe(true)
      expect(result.validation.errors.some(e => e.code === 'MISSING_STEPS')).toBe(true)
    })
  })
  ...
  describe('Recipe Validation', () => {
    it('should validate step dependencies', async () => {
      ...
      expect(validation.isValid).toBe(false)
      expect(validation.errors.some(e => e.code === 'UNKNOWN_DEPENDENCY')).toBe(true)
    })
    ...
    it('should detect duplicate step names', async () => {
      ...
      expect(validation.isValid).toBe(false)
      expect(validation.errors.some(e => e.code === 'DUPLICATE_STEP_NAME')).toBe(true)
    })
    ...
    it('should validate variable configurations', async () => {
      ...
      expect(validation.isValid).toBe(false)
      expect(validation.errors.some(e => e.code === 'INVALID_VARIABLE')).toBe(true)
    })
  })
...
describe('Template Variable Validation', () => {
  it('should validate string variables with patterns', () => {
    ...
    const valid = TemplateParser.validateVariableValue('testVar', 'ValidName', variable)
    const invalid = TemplateParser.validateVariableValue('testVar', 'invalid-name', variable)
  ...
  it('should validate number variables with min/max', () => {
    ...
    const tooHigh = TemplateParser.validateVariableValue('testVar', 101, variable)
  ...
  it('should validate enum variables', () => {
    ...
    const valid = TemplateParser.validateVariableValue('framework', 'react', variable)
    const invalid = TemplateParser.validateVariableValue('framework', 'svelte', variable)
