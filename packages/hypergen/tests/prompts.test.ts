...
import {
	InteractivePrompter,
	type PromptOptions,
} from '../src/prompts/interactive-prompts';
...
describe('Interactive Prompts System', () => {
	...
	describe('Parameter Validation', () => {
		it('should validate string parameters', () => {
			...
			// Valid string
			const validResult = prompter.validateStringInput(
				'ComponentName',
				variable,
			);
			...
			// Invalid pattern
			const invalidPatternResult = prompter.validateStringInput(
				'componentName',
				variable,
			);
			...
			// Too long
			const tooLongResult = prompter.validateStringInput(
				'A'.repeat(51),
				variable,
			);
		...
		it('should validate number parameters', () => {
			...
			// Invalid number
			const invalidResult = prompter.validateNumberInput(
				'not-a-number',
				variable,
			);
		...
		it('should validate array parameters', () => {
			...
			// Valid array
			const validResult = prompter.validateArrayInput(
				'item1, item2, item3',
				variable,
			);
			expect(validResult).toBeUndefined();
	...
	describe('Parameter Processing', () => {
		...
		it('should detect validation errors', () => {
			...
			expect(validation.errors).toContain(
				'framework must be one of: react, vue',
			);
		});
	});
	...
	describe('Multi-Select Enum Support', () => {
		...
		it('should validate multi-select enum values', () => {
			...
			expect(validation.valid).toBe(false);
			expect(validation.errors).toContain(
				'methods contains invalid values: PATCH, OPTIONS',
			);
		});
	});
