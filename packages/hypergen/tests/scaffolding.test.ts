...
describe('GeneratorScaffolding', () => {
	...
	describe('initGenerator', () => {
		it('should create a basic generator with actions and template', async () => {
			...
			expect(
				fs.existsSync(path.join(generatorDir, 'test-generator.test.ts')),
			).toBe(true);
		});
		...
		it('should create React component generator', async () => {
			...
			expect(
				fs.existsSync(
					path.join(generatorDir, 'templates', 'component.tsx.ejs'),
				),
			).toBe(true);
			expect(
				fs.existsSync(
					path.join(generatorDir, 'templates', 'component.test.tsx.ejs'),
				),
			).toBe(true);
			expect(
				fs.existsSync(
					path.join(generatorDir, 'templates', 'component.stories.tsx.ejs'),
				),
			).toBe(true);

			// Check actions.ts content
			const actionsContent = fs.readFileSync(
				path.join(generatorDir, 'actions.ts'),
				'utf-8',
			);
		...
		it('should create API endpoint generator', async () => {
			...
			expect(
				fs.existsSync(path.join(generatorDir, 'templates', 'route.ts.ejs')),
			).toBe(true);
			expect(
				fs.existsSync(
					path.join(generatorDir, 'templates', 'route.test.ts.ejs'),
				),
			).toBe(true);
			expect(
				fs.existsSync(path.join(generatorDir, 'templates', 'model.ts.ejs')),
			).toBe(true);

			// Check template.yml content
			const templateContent = fs.readFileSync(
				path.join(generatorDir, 'template.yml'),
				'utf-8',
			);
		...
		it('should create action-only generator', async () => {
			...
			expect(fs.existsSync(path.join(generatorDir, 'actions.ts'))).toBe(true);
			expect(fs.existsSync(path.join(generatorDir, 'template.yml'))).toBe(
				false,
			);
			expect(fs.existsSync(path.join(generatorDir, 'README.md'))).toBe(true);
		});
	...
	describe('initWorkspace', () => {
		it('should create workspace with examples', async () => {
			...
			expect(fs.existsSync(path.join(tempDir, 'hypergen.config.js'))).toBe(
				true,
			);
			...
			expect(fs.existsSync(path.join(tempDir, 'component', 'actions.ts'))).toBe(
				true,
			);
			expect(
				fs.existsSync(path.join(tempDir, 'component', 'template.yml')),
			).toBe(true);
			expect(fs.existsSync(path.join(tempDir, 'api-route', 'actions.ts'))).toBe(
				true,
			);
			expect(
				fs.existsSync(path.join(tempDir, 'api-route', 'template.yml')),
			).toBe(true);
			expect(
				fs.existsSync(path.join(tempDir, 'util-function', 'actions.ts')),
			).toBe(true);
			expect(
				fs.existsSync(path.join(tempDir, 'util-function', 'template.yml')),
			).toBe(true);
		});
	...
	describe('template content validation', () => {
		...
		it('should generate valid template.yml', async () => {
			...
			const templateYml = fs.readFileSync(
				path.join(tempDir, 'test-yaml', 'template.yml'),
				'utf-8',
			);
