...
describe("GeneratorScaffolding", () => {
	...
	describe("initGenerator", () => {
		...
		it("should create React component generator", async () => {
			...
			expect(fs.existsSync(path.join(generatorDir, "templates", "component.test.tsx.jig"))).toBe(
				true,
			);
			expect(fs.existsSync(path.join(generatorDir, "templates", "component.stories.tsx.jig"))).toBe(
				true,
			);
	...
	describe("template content validation", () => {
		it("should generate valid Jig templates", async () => {
			...
			const componentTemplate = fs.readFileSync(
				path.join(templateDir, "component.tsx.jig"),
				"utf-8",
			);
