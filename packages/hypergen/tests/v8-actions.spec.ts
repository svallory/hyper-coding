...
function createTestAction(
	metadata: any,
	implementation?: (context: ActionContext) => Promise<ActionResult>,
) {
	const impl =
		implementation ||
...
describe('ActionParameterResolver', () => {
	...
	it('should resolve parameters with defaults', async () => {
		...
		const resolved = await resolver.resolveParameters(metadata, {
			name: 'TestName',
		});
	...
	it('should validate all parameter types', async () => {
		...
		await expect(
			resolver.resolveParameters(metadata, {
				str: 'Valid',
				bool: 'not-boolean',
			}),
...
describe('ActionExecutor', () => {
	...
	it('should handle missing actions', async () => {
		await expect(executor.execute('nonexistent-action')).rejects.toThrow(
	...
	it('should stop sequence on failure', async () => {
		...
		const results = await executor.executeSequence(
			[{ name: 'success1' }, { name: 'failure' }, { name: 'success2' }],
			{ logger: new SilentActionLogger() },
		);
