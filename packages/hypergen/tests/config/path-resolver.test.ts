...
`)
  ...
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'component', 'cookbook.yml'), `\
name: "component"
defaults:
  recipe: "add"
`)
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'component', 'add', 'recipe.yml'), `\
name: component-add
steps: []
`)
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'component', 'remove', 'recipe.yml'), `\
name: component-remove
steps: []
`)

  // Cookbook: crud (no defaults)
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'crud', 'cookbook.yml'), `\
name: "crud"
`)
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'crud', 'resource', 'recipe.yml'), `\
name: crud-resource
steps: []
`)
  writeYaml(path.join(root, 'kits', 'nextjs', 'cookbooks', 'crud', 'list', 'recipe.yml'), `\
name: crud-list
steps: []
`)

  // -- Search directory ------------------------------------------------------
  writeYaml(path.join(root, 'search', 'my-cookbook', 'my-recipe', 'recipe.yml'), `\
name: my-recipe
steps: []
`)

  // -- Direct recipes search directory ---------------------------------------
  writeYaml(path.join(root, 'direct-recipes', 'simple', 'recipe.yml'), `\
name: simple-recipe
steps: []
`)

  // -- Simulated cwd with local files ----------------------------------------
  writeYaml(path.join(root, 'cwd', 'local-recipe.yml'), `\
name: local-recipe
steps: []
`)
  writeYaml(path.join(root, 'cwd', 'subdir', 'nested-recipe.yml'), `\
name: nested-recipe
steps: []
`)
}
...
describe('PathResolver', () => {
  ...
  describe('Kit-based resolution', () => {
    ...
    it('returns null when kit has no default cookbook and only kit name given', async () => {
      ...
      writeYaml(path.join(noDefaultKitDir, 'kit.yml'), `\
name: "bare-kit"
`)
  ...
  describe('Edge cases', () => {
    ...
    it('handles kit registered with empty cookbooks directory', async () => {
      ...
      writeYaml(path.join(emptyKitDir, 'kit.yml'), `\
name: "empty-kit"
cookbooks:
  - "./cookbooks/*/cookbook.yml"
defaults:
...
`)
    ...
    it('handles multiple kits in the map', async () => {
      ...
      writeYaml(path.join(secondKitDir, 'kit.yml'), `\
name: "react-kit"
cookbooks:
  - "./cookbooks/*/cookbook.yml"
`)
      writeYaml(path.join(secondKitDir, 'cookbooks', 'hooks', 'cookbook.yml'), `\
name: "hooks"
defaults:
  recipe: "add"
`)
      writeYaml(path.join(secondKitDir, 'cookbooks', 'hooks', 'add', 'recipe.yml'), `\
name: hooks-add
steps: []
`)
