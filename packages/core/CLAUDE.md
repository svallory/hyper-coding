...
This package provides the foundational types and systems used by all other HyperDev packages. It
contains NO CLI commands and NO runtime execution code - only types, configuration loading, error
...
- **hypergen-errors.ts** - Complete error system with 83 error codes
  ...
  - Error codes cover: config, templates, actions, codemods, file ops, discovery, URL resolution,
    network, AI
...
Note: DTS generation is disabled due to inflection types incompatibility. The package builds
successfully without type declarations.
