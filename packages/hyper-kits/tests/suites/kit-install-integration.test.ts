...
import { resolveKitSource, buildInstallCommand } from '#/source-resolver'
...
describe('Kit Install Integration Tests', () => {
  ...
  describe('Real-World Scenarios', () => {
    it('handles complex GitHub URLs with branches and tags', () => {
      ...
      expect(buildInstallCommand(branch, 'bun', {}))
        .toBe("bun add 'github:org/repo#feature-branch'")

      const tag = resolveKitSource('org/repo@v2.1.0')
      expect(tag.type).toBe('github')
      expect(buildInstallCommand(tag, 'npm', {}))
        .toBe("npm install 'github:org/repo@v2.1.0'")
    })
    ...
    it('handles monorepo local paths', () => {
      ...
      expect(buildInstallCommand(relative, 'bun', {}))
        .toBe("bun add '../../packages/my-kit'")
    })
    ...
    it('handles JSR packages for different package managers', () => {
      ...
      expect(buildInstallCommand(jsr, 'bun', {}))
        .toBe("bun add 'jsr:@std/path'")

      // Others use npx jsr add
      expect(buildInstallCommand(jsr, 'npm', {}))
        .toBe('npx jsr add @std/path')
      expect(buildInstallCommand(jsr, 'pnpm', {}))
        .toBe('npx jsr add @std/path')
      expect(buildInstallCommand(jsr, 'yarn', {}))
        .toBe('npx jsr add @std/path')
    })
  })
  ...
  describe('Error Cases', () => {
    it('rejects shell injection attempts', () => {
      ...
      expect(() => buildInstallCommand(malicious, 'bun', {}))
        .toThrow('Invalid kit specifier')

      const pipe = resolveKitSource('kit | cat /etc/passwd')
      expect(() => buildInstallCommand(pipe, 'npm', {}))
        .toThrow('Invalid kit specifier')
    })
