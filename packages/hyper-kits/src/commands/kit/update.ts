    ...
    switch (resolved.type) {
      ...
      case 'url': {
        ...
        if (resolved.source.endsWith('.tar.gz') || resolved.source.endsWith('.tgz')) {
          ...
          execSync(`curl -L -o "${tempZip}" "${resolved.source}" && unzip -q "${tempZip}" -d "${targetDir}" && rm "${tempZip}"`, {
            stdio: 'inherit',
          })
        } else {
          throw new Error('Unsupported archive format. Only .tar.gz, .tgz, and .zip are supported.')
        }
        break
      }
