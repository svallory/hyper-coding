...
function testEdgeCase(reflections, groupings, sidebarIcons, testName, expectedBehavior) {
  ...
  console.log(`Groupings: [${groupings.join(', ')}], Icons: ${Array.isArray(sidebarIcons) ? `[${sidebarIcons.join(', ')}]` : sidebarIcons}`);
...
for (const test of singleItemTests) {
  const passed = testEdgeCase(singleItem, test.groupings, test.icons, 
...
for (const test of sameFileTests) {
  const passed = testEdgeCase(sameFileItems, test.groupings, test.icons,
...
for (const test of inheritanceTests) {
  const passed = testEdgeCase(test.reflections, test.groupings, test.icons,
