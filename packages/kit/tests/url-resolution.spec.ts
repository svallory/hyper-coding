...
describe("URLCache", () => {
	...
	it("should verify cache integrity", async () => {
		...
		await fs.writeFile(
			path.join(cachePath, "template.yml"),
			"corrupted content",
		);
...
`;
	...
	it("should handle missing template files", async () => {
		await expect(resolver.resolve("./nonexistent", tempDir)).rejects.toThrow(
	...
	it("should resolve absolute paths", async () => {
		...
		await fs.ensureDir(templateDir);
		await fs.writeFile(
			path.join(templateDir, "template.yml"),
...
describe("GitHubResolver", () => {
	...
	it("should support GitHub URLs", () => {
		...
		expect(
			resolver.supports(
				"https://raw.githubusercontent.com/owner/repo/main/template.yml",
			),
		).toBe(true);
...
`;
	...
	it("should handle resolution errors gracefully", async () => {
		await expect(
			manager.resolveURL("./nonexistent-template", tempDir),
		).rejects.toThrow();
	});
