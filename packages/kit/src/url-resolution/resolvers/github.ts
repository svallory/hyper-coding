...
export class GitHubResolver implements TemplateURLResolver {
	...
	private parseGitHubShorthand(url: string): GitHubURLInfo {
		...
		if (parts.length < 2) {
			throw new Error(
				"Invalid GitHub URL format. Expected: github:owner/repo@version",
			);
		}
	...
	private validateSecurity(parsed: GitHubURLInfo): void {
		...
		if (
			...
			!this.security.allowedDomains.includes("github.com")
		) {
			...
			const request = https.get(
				...
				(response) => {
					...
					if (
						...
						contentLength > this.security.maxFileSize
					) {
						reject(
							new Error(`Template file too large: ${contentLength} bytes`),
						);
						return;
					}
					...
					response.on("data", (chunk) => {
						...
						if (
							this.security.maxFileSize &&
							receivedBytes > this.security.maxFileSize
						) {
							request.destroy();
							reject(
								new Error(`Template file too large: ${receivedBytes} bytes`),
							);
							return;
						}
