...
export default class KitInfo extends BaseCommand<typeof KitInfo> {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (kit.cookbooks && kit.cookbooks.length > 0) {
        ...
        try {
          const discoveredCookbooks = await discoverCookbooksInKit(
            kit.path,
            cookbookGlobs,
          );
      ...
      this.error(
        `Failed to get kit info: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private displayKitInfo(kitInfo: {
    ...
    // Title with version
    const versionStr = kitInfo.metadata?.version
      ? s.version(kitInfo.metadata.version)
      : "";
    ...
    if (kitInfo.metadata?.keywords && kitInfo.metadata.keywords.length > 0) {
      this.log(
        s.keyValue("Keywords", kitInfo.metadata.keywords.join(", "), 10),
      );
    }
  ...
  private suggestAvailableKits(kits: any[]): void {
    ...
    for (const [source, sourceKits] of bySource) {
      ...
      for (const kit of sourceKits) {
        const version = kit.metadata?.version
          ? s.version(kit.metadata.version)
          : "";
