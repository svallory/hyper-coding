...
export class GeneratorDiscovery {
  ...
  constructor(private options: GeneratorDiscoveryOptions = {}) {
    ...
    debug(
      "Using project root: %s (isMonorepo: %s)",
      this.projectRoot,
    ...
    const allDirs = [
      ...new Set([...defaultDirs, ...configDirs, ".hyper/kits"]),
    ];
  ...
  async discoverAll(): Promise<DiscoveredGenerator[]> {
    debug(
      "Starting generator discovery with sources: %o",
    ...
    await Promise.all(
      discoveries.map((gen) => this.enrichWithKitMetadata(gen)),
    );
  ...
  async discoverLocal(): Promise<DiscoveredGenerator[]> {
    debug(
      "Discovering local generators in directories: %o",
    ...
    for (const dir of this.options.directories || []) {
      ...
      const generatorGroups = this.groupFilesByGenerator(
        actionFiles,
        templateFiles,
        fullPath,
      );
  ...
  async discoverGlobal(): Promise<DiscoveredGenerator[]> {
    ...
    try {
      ...
      debug(
        "Found %d global packages installed: %o",
        globalPackages.length,
        globalPackages.map((p) => p.name),
      );

      for (const pkg of globalPackages) {
        // Filter by naming convention
        const isHyperKit =
          pkg.name.endsWith("-hyper-kit") ||
          pkg.name.startsWith("@kit/");
  ...
  private async checkAndAddGlobalPackage(
    packagePath: string,
    ...
    try {
      ...
      const isHyperKit =
        packageName.endsWith("-hyper-kit") ||
        packageName.startsWith("@kit/");
  ...
  private async findActionFiles(directory: string): Promise<string[]> {
    ...
    const patterns = this.options.patterns?.filter(
      (p) =>
        p.includes(".js") ||
        p.includes(".ts") ||
        p.includes(".mjs") ||
        p.includes("{js,ts,mjs}"),
    ) || ["**/*.{js,ts,mjs}"];
  ...
  private groupFilesByGenerator(
    ...
    const groups = new Map<
      string,
      { actions: string[]; templates: string[] }
    >();
  ...
  private async importModule(filePath: string): Promise<any> {
    try {
      ...
      debug(
        "Successfully imported module with exports: %o",
        Object.keys(module),
      );
  ...
  async registerDiscoveredActions(): Promise<void> {
    ...
    for (const generator of generators) {
      debug(
        "Looking for action files in generator: %s at path: %s",
        generator.name,
        generator.path,
      );
      const actionFiles = await this.findActionFiles(generator.path);
      debug(
        "Found %d action files for generator %s: %o",
        actionFiles.length,
        generator.name,
        actionFiles,
      );
  ...
  private async extractGeneratorMetadata(templateFile?: string): Promise<any> {
    ...
    try {
      ...
      debug(
        "Failed to parse template metadata from %s: %s",
        templateFile,
        error,
      );
      return undefined;
   ...
   */
  private async createWorkspaceKit(
    discoveries: DiscoveredGenerator[],
  ): Promise<DiscoveredGenerator | null> {
    debug("Creating virtual workspace kit for standalone cookbooks/recipes");

    const workspaceItems: { actions: string[]; templates: string[] } = {
      actions: [],
      templates: [],
    };
    ...
    const workspaceDirs =
      this.options.directories?.filter((dir) => dir !== ".hyper/kits") || [];
    ...
    for (const dir of workspaceDirs) {
      ...
      const standaloneActions = actionFiles.filter(
        (file) => !this.belongsToKit(file, kitPaths),
      );
      const standaloneTemplates = templateFiles.filter(
        (file) => !this.belongsToKit(file, kitPaths),
      );
    ...
    if (
      workspaceItems.actions.length > 0 ||
      workspaceItems.templates.length > 0
    ) {
      const actions = await this.extractActionsFromFiles(
        workspaceItems.actions,
      );

      debug(
        "Created workspace kit with %d actions and %d templates",
        actions.length,
        workspaceItems.templates.length,
      );
      ...
      return {
        ...
        metadata: {
          description:
            "Standalone cookbooks and recipes not belonging to any kit",
        },
      };
    }
   ...
   */
  private async enrichWithKitMetadata(
    generator: DiscoveredGenerator,
  ): Promise<void> {
      ...
      try {
        ...
        if (parsedKit.isValid) {
          ...
          if (config.cookbooks && config.cookbooks.length > 0) {
            ...
            generator.cookbooks = Array.from(cookbooks.values()).map(
              (c) => c.config.name,
            );
          }
          ...
          if (config.helpers) {
            ...
            if (await fs.pathExists(helpersPath)) {
              ...
              if (stats.isDirectory()) {
                ...
                // Single helper file
                generator.helpers = [
                  path.basename(config.helpers, path.extname(config.helpers)),
                ];
              }
            }
          }
    ...
    if (await fs.pathExists(packageJsonPath)) {
      try {
        ...
        generator.metadata = {
          ...
          description:
            generator.metadata?.description || packageJson.description,
