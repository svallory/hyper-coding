...
export class ActionTool extends Tool<ActionStep> {
	...
	protected async onValidate(
		...
		if (!actionInfo.exists) {
			...
			const similarActions = availableActions
				.filter(
					(name) =>
		...
		if (step.parameters) {
			try {
				const paramValidation = await this.executor.validateParameters(
					step.action,
					step.parameters,
				);
	...
	protected async onExecute(
		...
		try {
			...
			if (activeActionsResource?.metadata?.activeActions) {
				(activeActionsResource.metadata.activeActions as Set<string>).add(
					contextPrep.communication?.actionId || step.name,
				);
			}

			this.debug(
				"Action context prepared for %s: actionId=%s, params=%d",
				step.action,
			...
			this.debug(
				"Action %s completed successfully: %s (duration: %dms)",
				step.action,
	...
	private async prepareActionContext(
		...
		if (step.actionConfig?.communication) {
			communication = {
				actionId:
					step.actionConfig.communication.actionId ||
