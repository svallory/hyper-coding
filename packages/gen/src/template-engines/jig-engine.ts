 ...
 */
export function renderTemplateSync(template: string, context: Record<string, any>): string {
	const engine = getJig();
	try {
 ...
 */
export function registerHelpers(helpers: Record<string, any>, source?: string): void {
	const engine = getJig();
...
function registerAi2PassTags(edge: Edge): void {
	try {
		...
		debug(
			"@ai 2-pass tag registration failed: %s",
