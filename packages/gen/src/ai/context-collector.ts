...
export class ContextCollector {
	...
	async collect(
				...
				if (fs.existsSync(resolved)) {
					...
					if (currentTokens + tokens > maxTokens) {
						if (config.overflow === "truncate") {
							...
							throw new Error(
								`Context exceeds token budget (${currentTokens + tokens} > ${maxTokens}). File: ${filePath}`,
							);
						} else {
							bundle.truncated = true;
		...
		debug(
			"Context collection complete: %d files, %d configs, %d step outputs, ~%d tokens, truncated=%s",
			bundle.files.size,
			bundle.configs.size,
			bundle.stepOutputs.size,
			currentTokens,
			bundle.truncated,
		);
