...
export default class Run extends BaseCommand<typeof Run> {
  ...
  static override flags = {
    ...
    ask: Flags.string({
      description:
        "Who resolves missing variables: me (interactive), ai, or nobody (error)",
  ...
  async run(): Promise<void> {
    ...
    const askMode =
      (flags.ask as "me" | "ai" | "nobody" | undefined) ??
    ...
    try {
      ...
      if (collector.collectMode && collector.hasEntries()) {
        ...
        const promptTemplatePath =
          flags["prompt-template"] || aiConfig?.promptTemplate;
  ...
  private async executeGroup(
    ...
    try {
      ...
      const result = await groupExecutor.executeGroup(
        group,
      ...
      if (result.success) {
        ...
        for (const rr of result.recipeResults) {
          ...
          this.log(
            `  ${status} ${rr.name}: ${rr.result.metadata.completedSteps} steps`,
          );
        }
      } else {
        const errorMsg =
   ...
   */
  private async mapPositionalArgs(
    resolved: ResolvedPath,
    ...
    try {
      ...
      for (
        let i = 0;
        i < Math.min(resolved.remaining.length, positionalVars.length);
        i++
      ) {
