...
export default class RecipeList extends BaseCommand<typeof RecipeList> {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.error(
        `Failed to list recipes: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   */
  private quickParseRecipe(recipePath: string): {
    description?: string;
    version?: string;
    steps?: number;
  } {
  ...
  private displayRecipes(
    ...
    for (const [kitName, kitRecipes] of byKit) {
      ...
      for (const [cookbookName, cbRecipes] of byCookbook) {
        const prefix = cookbookName
          ? c.cookbook(`  ${cookbookName}/`)
          : c.subtle("  (direct)/");
