...
const CRUD_TEMPLATES_DIR = path.join(
  KIT_PATH,
  "cookbooks/crud/resource/templates",
);
...
describe("E2E: Next.js CRUD resource with 2-pass AI generation", () => {
  ...
  describe("Pass 1: Collect Mode", () => {
    ...
    it("should NOT create output files during collect mode", async () => {
      ...
      expect(
        await fs.pathExists(path.join(tempDir, "app/actions/product.ts")),
      ).toBe(false);
      // Verify no form component
      expect(
        await fs.pathExists(path.join(tempDir, "components/ProductForm.tsx")),
      ).toBe(false);
    });
  ...
  describe("Full Round-Trip", () => {
    it("should complete Pass 1 → Assemble → Pass 2 end-to-end", async () => {
      ...
      const actions = await fs.readFile(
        path.join(tempDir, "app/actions/product.ts"),
        "utf-8",
      );
      ...
      const form = await fs.readFile(
        path.join(tempDir, "components/ProductForm.tsx"),
        "utf-8",
      );
