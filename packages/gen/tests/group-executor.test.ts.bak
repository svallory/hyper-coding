...
import os from 'os'
import { GroupExecutor, type GroupRecipeEntry, type RecipeGroup } from '../../src/recipe-engine/group-executor'
import type { RecipeConfig } from '../../src/recipe-engine/types'
import type { RecipeExecutionResult, RecipeExecutionOptions, RecipeSource } from '../../src/recipe-engine/recipe-engine'
...
function createMockRecipeEngine(
  ...
  executeImpl?: (source: RecipeSource, options: RecipeExecutionOptions) => Promise<RecipeExecutionResult>,
) {
  return {
    loadRecipe: async (source: string | RecipeSource) => {
      const normalized = typeof source === 'string' ? { type: 'file' as const, path: source } : source
      ...
      return {
        ...
        source: normalized,
        validation: { isValid: true, errors: [], warnings: [], recipe: entry?.config as any, context: {} as any },
        dependencies: [],
      }
    },
    executeRecipe: executeImpl ?? (async (_source: string | RecipeSource, options: RecipeExecutionOptions = {}) => {
...
describe('GroupExecutor', () => {
  ...
  describe('buildDependencyGraph', () => {
    ...
    it('creates dependency when recipe A provides x and recipe B requires x', () => {
      const entries = [
        ...
        makeEntry('recipe-b', undefined, { x: { type: 'string', required: true } }),
      ]
    ...
    it('does NOT create dependency when recipe B has x with a default value', () => {
      const entries = [
        makeEntry('recipe-a', [{ name: 'x' }]),
        makeEntry('recipe-b', undefined, { x: { type: 'string', required: true, default: 'fallback' } }),
      ]
    ...
    it('builds a three-recipe chain: A provides x, B requires x & provides y, C requires y', () => {
      const entries = [
        makeEntry('recipe-a', [{ name: 'x' }]),
        makeEntry('recipe-b', [{ name: 'y' }], { x: { type: 'string', required: true } }),
        makeEntry('recipe-c', undefined, { y: { type: 'string', required: true } }),
      ]
    ...
    it('does not create self-dependency when a recipe provides and requires the same variable', () => {
      ...
      const entries = [
        makeEntry('recipe-a', [{ name: 'x' }], { x: { type: 'string', required: true } }),
      ]
  ...
  describe('circular dependency detection', () => {
    ...
    it('detects simple A <-> B circular dependency', () => {
      ...
      const entries = [
        makeEntry('recipe-a', [{ name: 'x' }], { y: { type: 'string', required: true } }),
        makeEntry('recipe-b', [{ name: 'y' }], { x: { type: 'string', required: true } }),
      ]
    ...
    it('detects A -> B -> C -> A circular dependency', () => {
      ...
      const entries = [
        makeEntry('recipe-a', [{ name: 'a' }], { c: { type: 'string', required: true } }),
        makeEntry('recipe-b', [{ name: 'b' }], { a: { type: 'string', required: true } }),
        makeEntry('recipe-c', [{ name: 'c' }], { b: { type: 'string', required: true } }),
      ]
  ...
  describe('executeGroup', () => {
    ...
    it('stops execution on failure when continueOnError is false (default)', async () => {
      ...
      const entries: GroupRecipeEntry[] = [
        makeEntry('recipe-a'),
        makeEntry('recipe-b'),
        makeEntry('recipe-c', undefined, { x: { type: 'string', required: true } }),
      ]
    ...
    it('accumulates providedValues across batches', async () => {
      ...
      const result = await executor.executeGroup(group, { baseKey: 'baseValue' })
  ...
  describe('loadGroupConfigs', () => {
    it('loads configs for entries that do not already have one', async () => {
      const entries: GroupRecipeEntry[] = [
        ...
        { name: 'recipe-b', recipeYmlPath: '/fake/recipe-b/recipe.yml', config: { name: 'already-loaded', steps: [], variables: {} } as RecipeConfig },
      ]
      ...
      const mockEngine = {
        loadRecipe: async (source: any) => {
          ...
          return {
            recipe: { name: path.basename(path.dirname(source.path)), steps: [], variables: {} } as RecipeConfig,
          }
        },
      } as any
  ...
  describe('edge cases', () => {
    ...
    it('handles a required variable not provided by any sibling (no dependency, just external)', () => {
      const entries = [
        makeEntry('recipe-a'),
        makeEntry('recipe-b', undefined, { orphanVar: { type: 'string', required: true } }),
      ]
