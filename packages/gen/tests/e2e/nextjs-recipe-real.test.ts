...
describe("E2E: Real Next.js Recipe", () => {
  ...
  beforeEach(async () => {
    ...
    const { ToolRegistry } = await import(
      "../../src/recipe-engine/tools/registry"
    );
    ToolRegistry.reset();
  ...
  it.skip("should execute nextjs project create recipe with nested sequences", async () => {
    ...
    expect(await fs.pathExists(path.join(projectDir, "next.config.ts"))).toBe(
      true,
    );
    expect(
      await fs.pathExists(path.join(projectDir, "app", "layout.tsx")),
    ).toBe(true);
    expect(await fs.pathExists(path.join(projectDir, "app", "page.tsx"))).toBe(
      true,
    );
    expect(
      await fs.pathExists(path.join(projectDir, "public", "next.svg")),
    ).toBe(true);
    expect(
      await fs.pathExists(path.join(projectDir, "eslint.config.mjs")),
    ).toBe(true);
  });
});
